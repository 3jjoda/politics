-- ⚠️ **이 마이그레이션은 같은 날 `2026-08-12-politician-titles.sql` 로 대체됐다.**
--    한 사람이 여러 직위를 동시에 갖는다는 걸 뒤늦게 확인해(한병도 = 원내대표 + 당대표 직무대행 +
--    국회운영위원장) 컬럼에서 테이블로 옮겼고, 그 마이그레이션이 여기서 만든 컬럼을 DROP 한다.
--    순서대로 실행하면 "추가 후 삭제" 라 최종 결과는 동일하다 — 이력 보존을 위해 파일은 남긴다.
--    새로 붙일 직위는 `politician_titles` 테이블에 넣을 것.
--
