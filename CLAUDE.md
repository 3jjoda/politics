...
| `/community` | `community/list.ejs` | 게시판 목록 (20개/페이지). **2026-08-19 보드(행) 형태 + 글 유형 탭** (`?type=` — 전체·공지·잡담·법안 이야기·질문·건의·피드백, 모르는 값은 전체). 행 = `[유형] 제목 [댓글수] (법안칩) | 닉네임·시간·조회`, 고정 공지는 `is_pinned DESC` 로 맨 위 + 골드 배경. 카드형에서 바꾼 이유: 썸네일·요약이 없어 글 하나가 110px 이었다 |
