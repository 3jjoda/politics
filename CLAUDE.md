...
- 🔴 **공유/저장은 재진입 차단(`busy`) + PNG 를 그릴 때 미리 만들어(`prepareBlob`) 클릭 순간 await 없이 시트를 연다** — 폰에서 시트가 열렸다 닫혔다
  ...
  🔴 **삼성 인터넷 + 넓은 화면(폴드 펼침·태블릿, `innerWidth ≥ 600`)은 공유 시트를 건너뛰고 저장으로** — 시트가 떠 있는 창으로 뜨면서 재생성을 반복했다
  (2026-08-16 갤럭시 폴드 실기기 영상. 접은 화면에선 정상). 삼성 쪽 동작이라 코드로 못 막는다. 실패한 적이 있으면 좁은 화면도 저장으로
