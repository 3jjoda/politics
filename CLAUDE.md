...
- 관련 문서:
  ...
  - [SNS.md](./SNS.md) — **SNS 운영 전략** (4축 콘텐츠 엔진·일일 루틴·30일 플랜·측정 지표. 2026-08-18)
  - [ELECTION_LAW.md](./ELECTION_LAW.md) — **선거법 검토** (선관위 질의 대상 기능 5등급·질의 문항 초안·선거기간 대비. 2026-08-27)
...
#### 🔴 로그인 페이지는 세션을 쓰지 않는다 (2026-08-18)
`renderLogin` 이 `req.session.authNext = next` 를 쓰고 있었다. **로그인 페이지를 보기만 해도**
세션이 초기화돼 `saveUninitialized: false` 가 무력해지고 **요청마다 DB 행이 하나씩** 생긴다.

법안 상세마다 `/auth/login?next=/bill/…` 링크(분석 요청 위젯)가 있어 **크롤러가 그걸 따라가면 폭증한다** —
실측 익명 세션 8,599건 중 **8,553건(99.5%)이 법안 상세 next**, 08-14 크롤러 사건 당일에만 4,805건.

- → `next` 는 **로그인 화면의 provider 링크 쿼리**로 넘긴다 (`/auth/google?next=…`).
  세션 쓰기는 실제로 OAuth 를 시작하는 `routes/AuthRoutes.js` 진입 미들웨어에서만
- ⚠️ **오픈 리다이렉트 방어**: 뷰에서 `/…` 로 시작하는 내부 경로만 통과시킨다.
  `//호스트` 는 외부이므로 `(?!/)` 로 막는다
- ⚠️ `saveUninitialized: false` 는 정상 동작하고 있었다 — "아무것도 안 쓴" 세션만 거른다는 뜻이라,
  우리가 쓰는 순간 대상에서 빠진다. **설정이 아니라 쓰기 지점의 문제**였다
- 기존 행은 만료(14일)되면 앱이 요청마다 도는 `DELETE FROM session WHERE expire < now()` 로 자동 정리된다
- 실측: 로그인 페이지 10회 → 세션 증가 **0** / `/auth/google?next=` 1회 → **+1** (의도대로)
...
- `politician_speeches` — **의원 발언 기록** (2026-08-12 적재 · 2026-08-15 배치 재작성 · 2026-09-01 파서·분류 재작성, `batch/syncSpeeches.js`). 실측 **68,483행 / 의원 309명 / 2024-06-05~2026-08-31**
  ...
  - **`role_kind` 5종** (2026-08-15 구 `gov` 3분할, `ddl/migrations/2026-08-15-speeches-role-kind-split.sql`):
    ...
    | 값 | 실측 (2026-09-01) | 뜻 | 화면 |
    |---|---|---|---|
    | `member` | 41,202 | 질의석 — 위원·간사·의원 | ✅ |
    | `chair` | 21,598 | 위원장석(사회) — 안건 호명 | ✅ (분리 표시) |
    | `government` | 5,351 | 정부측 (장관·차관·청장·**정부 위원회 위원장**…) | ❌ |
    | `other` | 329 | 도지사·교수·회장·사장 등 | ❌ |
    | `witness` | 3 | 참고인·증인·진술인 | ❌ |

  - 🔴 **집계는 `member`·`chair` 만 한다.** 소스가 MONA_CD 를 주지 않아 **이름으로만 매칭**하기 때문이다
    (실측 오귀속: 도지사 김영환 87건 · 회장 김병주 21건 · 변호사 김종민 15건 · 교수 박은정 7건 — 전부 다른 사람)
  ...
  - 🔴 **`chair`·`member` 판정은 직위가 아니라 `직위 + 소속(org)` 이다** (2026-09-01, `ddl/migrations/2026-09-01-speeches-org.sql`).
    이 문서에 오래 적혀 있던 *"`위원장`·`위원`·`간사` 는 국회의원만 가질 수 있어 이름 충돌이 구조적으로 없다"* 는 **틀렸다.**
    정부·독립기관에도 `위원장`·`위원` 이 있다 — 금융위 518 · 방송통신위 494 · 공정거래위 306 · 국가인권위 162 · 중앙선관위 149 …
    (국회 직위 문자열인데 소속이 정당이 아닌 것 **실측 2,854건**)
    - **실제 사고**: `이진숙 위원장(방송통신위원회) 답변` 192건이 **국회 상임위원장 사회 발언**으로 집계됐다.
      김태규(방통위 직무대행) 227건도 같다. 두 사람 모두 나중에 의원이 되어 이름이 매칭됐다.
      화면은 이진숙 의원에게 `위원장석 발언 192건 · 회의 진행·안건 호명` 이라고 **거짓을 말했고**,
      기간도 취임(2026-06-04) 2년 전인 `2024-08-14~` 로 나왔다. chair 는 집계 대상이라
      **상임위 참여율의 분모·분자까지 오염**됐다 (이진숙 `86번 중 11번 = 12.8%` → 실제 `2번 중 2번`)
    - → `classifyRole(role, org)`: 국회 직위여도 **소속이 정당이 아니면 `government`**.
      소속이 없으면(괄호 없음) 그대로 인정한다 — 국회의장·부의장은 당적을 이탈해 소속 표기가 없다 (실측 chair 1,015건)
    - ⚠️ 정당 판정은 `parties` **∪ `PARTY_FALLBACK` 상수**다. `parties` 는 현재 정당만 담아서
      임기 중 사라진 정당(실측 `새로운미래` 10 · `국민의미래` 1)이 빠지고, 그것만 쓰면 그 소속 의원이 통째로 외부인이 된다
    - ⚠️ 그래도 못 채운 정당이 생길 수 있어 배치가 **`[소속미상]` 로그**로 강등된 소속을 전부 찍는다.
      거기 **정당 이름이 뜨면 `PARTY_FALLBACK` 에 넣을 것** (기관명만 뜨는 게 정상)
  - 🔴 **파싱은 정규식 한 방이 아니라 괄호를 경계로 자른다** (2026-09-01). 원문은
    `이름 + 직위(1~5어절) + (소속) + 행위(쉼표 여러 개)` 인데, 구 `SEG_RE` 는 직위를 **한 어절**로만 봐서
    직위가 두 어절이면 **소속을 통째로 놓치고 수식어를 행위로 먹었다**:
    `"김태규 위원장 직무대행(방송통신위원회)  답변"` → `직위=위원장 · 소속=없음 · 행위=직무대행` (실측 544건).
    소속이 사라지면 위 판정이 성립하지 않으므로 **파서 수정이 소속 기반 분류의 전제**였다
    - 괄호가 없으면(6.4%) **두 칸 이상 공백**이 직위와 행위를 가른다
    - 소속 괄호는 **마지막** 것으로 잡는다 — `"김상준 (전)정보통신기획평가원 …"` 처럼 앞에 붙는 괄호가 있다 (22건, 전부 외부인)
    - 부수효과로 파싱률 **98.6% → 99.8%**, 남은 실패는 전부 `"실무자  답변"` 처럼 이름이 없는 것
    - ⚠️ `role` 이 여러 어절이 되어 컬럼을 **VARCHAR(20) → (40)** 으로 넓혔다. `act` 는 20자로 자른다
      (`"개의, 발언, 의사일정 제1항 상정"` 같은 값이 온다)
...
- `etc/ddl/migrations/` — 누적 변경
  ...
  - `ddl/migrations/2026-08-23-bill-text-search-trgm.sql` — **쟁점 키워드 → 법안 매칭용 trigram 인덱스**. `pg_trgm` + `idx_bills_summary_trgm`(36MB) · `idx_bills_bill_name_trgm`(2MB). 뉴스에 나오는 말(`법원행정처`·`전세사기`)로 법안을 찾으려면 `bill_name` 만으론 안 된다 — 87%가 동명이라 이름엔 쟁점이 없고 내용은 `summary` 에만 있다
    - 🔴 **한글에서 `to_tsvector` 는 부분일치를 구조적으로 못 한다.** 한국어 사전이 없어 `simple` 을 쓰는데 교착어라 조사가 붙은 채 한 토큰이 된다 — `to_tsvector('simple','법원행정처를 폐지하고')` → `'법원행정처를'`, 그래서 `plainto_tsquery('법원행정처')` 가 **false**. 접두(`:*`)로도 중간 일치는 못 잡는다. **trigram 이어야 한다**
    - ⚠️ 그래서 기존 `idx_bills_bill_name`(GIN tsvector)은 이 프로젝트 검색이 전부 `ILIKE '%…%'` 라 **한 번도 쓰인 적이 없다.** 지우진 않았지만 ILIKE 를 가속한다고 오해하지 말 것
    - 🔴 **OR 는 양쪽이 다 인덱싱돼야 한다** — `bill_name ILIKE … OR summary ILIKE …` 에서 한 갈래라도 인덱스가 없으면 BitmapOr 를 못 만들고 **전체가 Seq Scan** 이 된다. summary 만 걸면 그 인덱스가 실제 쿼리에서 안 쓰인다. 그래서 둘을 같이 걸었다
    - 실측: summary `법원행정처` 536ms → **9.5ms** · `전세사기` 617ms → 12.0ms · bill_name `조세특례` 155ms → 14.8ms · **OR 결합 BitmapOr 1.5ms**. CONCURRENTLY 가 Transaction Pooler(6543)에서도 통과 (summary 16.9초)
    - ⚠️ **2글자는 폴백한다** (`국방` → Seq Scan 902ms). trigram 이 안 만들어지는 길이라 정상 — 쟁점어를 2글자로 잡지 말 것
    - 🔴 **OR 는 갈래가 하나라도 비면 전부 Seq Scan 이다.** `/bill?search=` 의 3중 OR(`bill_name`/`proposer_name`/`bill_no`)에서 갈래를 하나씩 빼며 실측: name 단독 3.2ms → +proposer **BitmapOr 2.4ms** → +`bill_no` **69ms Seq Scan** → `idx_bills_bill_no` 추가 후 3.8ms. `bill_no`(7자 고정·전건 고유)에 인덱스가 아예 없었다. **플래너는 모든 갈래에 인덱스 경로가 있을 때만 BitmapOr 를 만든다** — OR 에 컬럼을 더할 땐 그 컬럼 인덱스부터 볼 것
    - `/bill?search=` 실측 123ms → **1.1~3.8ms**. `idx_bills_proposer_name_trgm`(400kB) · `idx_bills_bill_no`(432kB, UNIQUE 아님 — 원천 중복이 배치를 깨뜨리지 않게)
...
지역구는 로그인 후 **클릭 두 번**이라 장벽이 압도적으로 낮고, "당 말고 사람" 축에 정확히 맞는다
(내가 뽑은 사람이 지금 뭐 하고 있나).

```
ddl/migrations/2026-08-23-user-district.sql   ← users.district VARCHAR(60)
daos/queries/district/*.sql · daos/DistrictDao.js · services/DistrictService.js
PUT /api/auth/district        ← 등록·변경·해제 (requireLogin)
/my #district                 ← 시도 optgroup + 지역구 select (JS 없이도 폼)
/ (홈)                        ← 무작위 3명 **위**에 「내 지역구 의원」 카드
```

- 🔴 **mona_cd 가 아니라 지역구 문자열을 저장한다.** 선거·보선으로 사람이 바뀌어도 "내 지역구" 는 유지돼야 한다.
  실측: 한 지역구에 현직 1 + 퇴임 1 인 곳이 **8군데** (울산 남구갑 = 김태규 / 김상욱(퇴임) 등).
  의원은 조회 시점에 `active_yn DESC` 로 찾는다
- 🔴 **값은 DB 화이트리스트로 검증한다** (`DistrictService.isValid`). 임의 문자열을 저장하지 않는다.
  실측 거부: 없는 지역구 · `비례대표` · 빈 값
- 🔴 **비례대표(46명)는 목록에서 뺀다** — 지역구가 아니다
- ⚠️ **시도는 파싱이 아니라 보정이다.** 대부분 `첫 어절 + 공백` 이지만 예외가 있다 —
  `세종특별자치시갑`/`을` 은 **공백이 없고**(2건), `전남광주통합특별시 광산구갑` 은 첫 어절이 통합 행정구역명이라 길다(18건, 정상).
  새 예외가 생기면 `getDistrictList.sql` 의 CASE 한 줄. 실측 **255 지역구 · 17 시도**
- ⚠️ 사용자가 **스스로 고른 값**이라 성별·연령대와 같은 성격이다 (개인정보처리방침 회원 정보 항목).
  우리가 관찰한 열람 기록(어느 의원 페이지를 봤나)과는 다르다 — 그건 여전히 만들지 않는다
- ⚠️ **홈은 로그인 사용자에게만 그린다.** 비로그인에게 로그인 유도를 하나 더 붙이지 않는다.
  등록 안 한 로그인 사용자에게만 한 줄 CTA(`/my#district`)
- ⚠️ 저장 후 **부분 갱신하지 않고 새로고침**한다 — 지역구가 바뀌면 의원(이름·정당·링크)이 통째로 달라져 옛 링크가 남는다
- ⚠️ 목록은 **6시간 캐시** (선거 때만 바뀐다). `findById.sql` 에 `district` 를 추가해 `currentUser.district` 로 온다
- 🔴 **비로그인도 고를 수 있다** (2026-08-23). 로그인을 요구하면 "진단(20문항)보다 장벽이 낮다" 는
  이 기능의 존재 이유가 사라진다. 공개 API 2종 — `GET /api/district/list` · `GET /api/district/member?d=`
  - 🔴 **비로그인 선택은 `localStorage`(`pb.district`)에만 남고 서버로 가지 않는다.** 정치 사이트에서
    "이 방문자의 지역구" 를 서버에 쌓지 않겠다는 뜻이다 (방문 통계가 개인 열람 기록을 안 남기는 원칙과 같은 판단).
    로그인 사용자만 `users.district` 에 저장한다 — 본인이 명시적으로 준 값이라 성격이 다르다
  - 🔴 **로그인했는데 서버에 없고 브라우저에만 있으면 한 번 올려준다** — 비로그인으로 고른 뒤 로그인한 사람에게
    다시 고르라고 하면 안 된다. 승격 성공 시 localStorage 를 지운다
  - ⚠️ **저장값이 무효면(행정구역 개편으로 사라짐) 조용히 버리고 다시 고르게 한다.** API 가 404 를 주므로
    화면이 구분할 수 있다. 실측 확인: `없어진구` → 카드 미생성 · 선택 UI 노출 · localStorage 정리
  - 🔴 **카드에 `변경` 버튼이 있어야 한다.** 카드만 그리고 끝내면 **비로그인은 영영 못 바꾼다**
    (`/my` 에 못 가므로 localStorage 를 직접 지우는 수밖에 없다 — 사용자 지적).
    상태 셋을 한 컨테이너(`#mine-wrap`)가 관리한다: **미등록(피커) / 등록됨(카드) / 변경중(피커)**.
    변경 화면은 `지금은 경기 광주시갑 입니다` 로 현재 값을 말하고 select 를 그 값으로 미리 고른다.
    `등록 안 함` 을 고르면 해제된다 (비로그인은 localStorage 삭제 · 로그인은 서버 NULL)
  - 🔴 **카드를 `<a>` 로 감싸지 말 것** — 안에 버튼이 들어가는데 중첩 인터랙티브가 안 된다.
    컨테이너는 `div`, 링크는 안쪽 `<a>` 둘(이름·기록 보기). `.same-name-chip` 에서 겪은 제약과 같다
  - ⚠️ **서버 렌더 카드와 JS 가 그리는 카드의 마크업이 같아야 한다** — 다르면 로그인/비로그인이 달라 보인다
  - ⚠️ 선택 UI 는 **JS 로만 그린다**(`hidden` 을 JS 가 푼다). 목록 255개를 SSR 하면 홈이 7KB 무거워지는데
    대부분의 방문자는 이미 등록했거나 안 쓴다
- 🔴 **`/my` 지역구 필드는 `grid-column: 1 / -1`(전체 폭) + `.pf-field { min-width: 0 }`** 이어야 한다.
  안 그러면 **카드가 깨진다** — 옵션 이름이 길어(`전남광주통합특별시 순천시광양시곡성군구례군갑`)
  select 가 그리드 트랙을 밀어낸다. 실측 재현: `repeat(auto-fit, minmax(180px,1fr))` 가
  **`285px·285px·285px·0px` 4열로 무너짐** → 수정 후 `208.5px × 4` 균등
- 🔴 **`.pf-label` 에서 mono + 자간을 뺐다** (2026-08-23). JetBrains Mono 는 한글 글리프가 없어 폴백되는데
  `letter-spacing: 0.06em` 까지 겹쳐 `내  지역구` 로 흩어졌다 (사용자 스크린샷).
  `/xray/chart` 의 `.ch-label` 에서 이미 겪은 함정 — 라벨이 전부 한글이라 mono 를 쓸 이유가 없다.
  ⚠️ 이 변경은 마이페이지 **모든 필드 라벨**에 적용된다 (계정·이메일·성별·가입일)
- 다음: 「지난달과 달라진 점」은 스냅샷 이력이 필요해 미착수 (`politician_committee_history` 선례 있음)
...
| `/admin/sns` | `admin/sns.ejs` | **관리자 — SNS 콘텐츠 허브** (2026-08-26, `requireAdmin`). 캐러셀 3종(`/promo/intro`·`numbers`·`balance`) + 쓰레드 문안 뱅크(`/promo/threads`) + 최근 브리핑 7건의 인스타 카드·쓰레드 체인 링크(폴백·활동없음은 `올리지 않는 카드` 표시) + 주간 루틴 요약. 🔴 도구 페이지 자체는 공개 라우트 — 여기는 찾아가는 메뉴지 권한 경계가 아니다. 전략 원문은 SNS.md. 진입: 계정 메뉴 `SNS 콘텐츠`. **+ 성과표** (2026-08-27): `sns_log` 테이블(`ddl/migrations/2026-08-27-sns-log.sql`) + 같은 화면의 기록 폼·행별 `채우기`(지표는 금요일)·축별/주간 집계 — PRG 패턴(titles 와 동일), 팔로워 수 컬럼은 일부러 없다. 게시일은 폼 입력값 그대로 (서버 CURRENT_DATE 안 씀). 브리핑 행에 **+기록 인스타/쓰레드** 원클릭 버튼 — 게시일은 브리핑 날짜가 아니라 **누른 날**(다음 날 아침에 올리므로), 포맷에 브리핑 날짜를 남긴다. ⚠️ `briefing_date` 는 TO_CHAR 로 받는다 (raw 면 JS Date 가 돼 화면에 영문 날짜가 찍힌다).
🔴 **몰아서 소급 입력하면 전부 「누른 날」로 들어간다** (2026-08-27 실제 발생) — 08-13~08-24 브리핑 14건을 한 번에 넣었더니
posted_on 이 전부 08-27 이 됐다. 원클릭 버튼은 **그날 올린 것을 그날 기록**할 때만 맞다. 소급 입력은 폼에서 날짜를 직접 고를 것.
⚠️ 이미 틀어진 기록은 **게시물에서 되찾을 수 있다** — 아래 「SNS 게시 시각 되찾기」 참조.
🔴 **짝 없는 브리핑 경고** (2026-08-27) — 브리핑은 인스타·쓰레드에 같이 올리므로 한쪽만 기록돼 있으면 빠뜨린 것이다.
포맷의 `(MM-DD)` 로 묶어 `HAVING NOT (인스타 AND 쓰레드)` 로 잡는다. 실제로 08-12 브리핑의 쓰레드 행이 통째로 빠져 있었는데 눈으로는 못 잡았다.
⚠️ **패턴에 역슬래시를 쓰지 말 것** — 템플릿 리터럴이 먹어서 매칭이 통째로 실패하고 경고가 **조용히 안 뜬다**. 대괄호(`[0-9]`)라야 안전하다.
🔴 **채널별 입력 안내 4칸**(`.lg-help`)이 폼 위에 있다: 쓰레드는 **저장이 없어 비우고**, **도달 ← 조회수**, **답글 ← 답글 − 내가 단 것**.
체인의 2·3번 글이 1번의 답글로 잡히므로 브리핑 체인의 실제 답글은 보통 **0**이고, 지표는 **1번(훅)에서만** 읽는다.
⚠️ 인스타 **도달**(본 계정 수)과 쓰레드 **조회수**(노출 횟수)는 다른 지표다 — 한 칸에 들어가도 채널을 넘어 평균 내지 말 것.
🔴 **집계는 채널 × 축이다** (2026-08-27). 채널마다 채우는 칸이 달라 섞으면 평균이 곧바로 거짓이 된다 —
실제로 쓰레드 행에 `저장 0` 이 들어가 `오늘` 축 평균 저장이 내려가 있었다. **쓰레드·유튜브의 `saves` 는 NULL 로 둔다**
(0 은 "쟀는데 없음" 이라 평균에 들어가고, NULL 은 자연히 빠진다).
⚠️ 집계가 `COUNT(지표)` 를 같이 내고 화면이 `n=` 으로 밝힌다 — 게시 9건인데 평균이 1건에서 나온 값일 수 있다. **n 이 1~2면 순위로 읽지 말 것** |
...
- 🔴 **안보축은 0 이 아니라 NULL 이고, 화면은 이유를 숫자로 쓴다.** 0 으로 두면 "둘 다 중도" 로 읽혀 거짓이 된다.
  `_profile_vs.ejs` 의 안보 행을 **지우지 않고** `분류 18,590건 중 자주 72건 · 동맹 268건` 을 쓴다 (`axisConfig.UNMEASURED_STATS`,
  2026-08-19 재측정 — 08-16 저녁 재분류가 사회·제도 일부를 안보로 옮겼는데 상수를 안 올려 화면·홍보글이 사흘간 옛 값(자주 59 · 동맹 253)을 말했다.
  🔴 **재분류하면 반드시 같이 갱신할 것** — 이건 주석이 아니라 화면 문구다). 빈 자리는 고장으로 읽힌다
...
#### 홈 좁히기 — 길이가 아니라 **초점**이었다 (2026-08-23)
외부 평가가 "홈이 너무 많은 것을 보여준다" 고 했는데 실측해 보니 **길이는 이미 짧았다**
(데스크톱 3.3화면 · 모바일 4.6화면 · 섹션 4개 — 2026-08-16 재구성에서 이미 6→4로 줄였다).
진짜 문제는 **미등록 첫 화면(812px)에 행동 요소가 8개**이고, 그중 **장벽이 가장 낮은
`지역구 고르기` 가 487px 아래에 점선 상자**로 있었다는 것이다 (성향 진단은 로그인+20문항인데 완료 3명).

- 🔴 **미등록 지역구 피커를 첫 행동으로 강조** — 골드 테두리 + 골드 배경 + 채운 버튼.
  점선 회색 상자는 "나중에 해도 되는 것" 으로 읽힌다
- 🔴 **무작위 의원 `getAxisSpotlight(3)` → `(2)`.** 이 블록이 **모바일 히어로의 절반**(750/1,402px)을
  먹는데 무작위라 재방문 가치가 낮다. 2명이면 "사람마다 다르다" 는 그대로 보인다.
  ⚠️ **1명으로 줄이지 말 것** — 비교 대상이 없으면 "정당 평균과 얼마나 다른가" 가 안 읽힌다
- 실측: 모바일 3,677 → **3,486px**(4.6→4.3화면) · 데스크톱 2,651 → **2,500px**(3.3→3.1화면) ·
  히어로 1,402 → 1,210px(모바일) · 데스크톱 첫 화면 행동 요소 **5개**
- ⚠️ **nav 중복 링크(의원·법안·차트·읽는 법)를 모바일에서 빼지 말 것** — 데스크톱은 숨기지만
  모바일은 nav 가 햄버거라 이게 유일한 표면 경로다 (2026-08-16 판단 유지)
- ⚠️ **더 줄이려 하지 말 것.** 3.1화면짜리 홈에서 섹션을 더 빼면 "이 사이트가 뭘 하는지" 가 안 보인다.
  평가의 "많다" 는 **선택지 수**를 말한 것이지 스크롤 길이가 아니었다

#### 🔴 홈 4차 재구성 — **A → B → C** (2026-08-25). 히어로는 「좌표 위의 당신」이다
사용자 지적: *"컨텐츠는 좋은데 시선을 사로잡지 못한다. 이러이러한 게 가능하다는 느낌이 확 와서
해보고 싶은 마음이 드는 스토리텔링이면 좋겠다."* 실측 진단 셋:

| | 무엇이 문제였나 |
|---|---|
| ① | 히어로 **375px 이 전부 텍스트**였다 (날짜줄·헤드라인·문단·링크). 이 사이트가 뭘 할 수 있는지가 **한 번도 시연되지 않았다** |
| ② | 여섯 섹션이 전부 같은 리듬(라벨 → 제목 → 흰 카드 → 회색 각주). 크기·색·형태 어디에도 대비가 없었다 |
| ③ | 가장 강한 자산이 전부 아래에 작게 있었다. 결론 3숫자는 **문서 맨 끝** 스트립, 좌표 지도는 **홈에 아예 없었다** |

→ **세 자산을 각각 한 박자로 세우고 순서로 이야기를 만든다.** 셋은 경쟁이 아니라 서로 다른 질문에 답한다:

| 박자 | 섹션 | 답하는 질문 |
|---|---|---|
| **A** | 히어로 — 좌표 지도 269점 + **첫 문항 하나** | 당신은 어디에? |
| **B** | 「국회는 이렇게 굴러갑니다」 3숫자 + 다크 밴드 | 국회는 어떤 곳인가? |
| **C** | 「오늘의 1면」 브리핑 리드 기사 | 오늘 무슨 일이 있었나? |
| 그다음 | 내 지역구 · 맞는/다른 의원 · 스포트라이트 · 숫자로 본 국회 | (기존 구조 유지) |

- 🔴 **A — 히어로가 진단 퍼널의 첫 칸을 먹는다.** 실측 완료자가 **5명**(2026-08-25 기준선)인데, 원인은 "진단하면 좌표가
  나옵니다" 라는 **문장 하나로 20문항을 시작하게 한 것**이었다. 지금은 첫 문항을 히어로에서 바로 묻고,
  누르면 **지도에 임시 점이 찍힌다**. 답변은 `localStorage('pb.bg.answers')` → `POST /api/balance-game/answers`
  (비로그인은 채점만·DB 쓰기 0 / 로그인은 그 POST 가 곧 저장이라 `respond` 가 **2번 문항부터** 이어진다).
  실측 확인: 홈에서 Q1 답변 → `/balance-game/respond` 가 `2 / 20` 으로 열린다
  - 🔴 **완료자에게 1번 문항을 보여주지 말 것.** 그 자리에 **내 좌표**(유형 이름 + 가장 가까운 의원 + 링크 4개)가 온다.
    `/auth/welcome` 에서 겪은 것과 같은 실수다 (방금 5분 쓴 사람에게 "20문항 · 약 5분")
  - 🔴 **시작만 하고 안 끝낸 사람도 구분한다** — `res.locals.balanceGameAnswered`(middlewares/balanceGame.js,
    기존 행에 컬럼 하나 추가라 조회는 안 늘었다). 비로그인은 화면이 localStorage 로 같은 판정을 한다
  - 🔴 **지도가 주인공, 문항은 곁이다.** 순서가 뒤집히면 설문 사이트로 보인다
  - 🔴 **점에 정당색을 입히지 말 것** — 두 덩어리로 갈리는 게 보이는데 색을 주면 그게 곧 정당 대비 구도가 된다
    (공유 카드 지도형과 같은 규칙). 익명 회색이라 성립하는 그림이다
  - 🔴 **임시 점은 "문항 1개 기준 임시 위치" 라고 범례에 쓴다.** 안 쓰면 20문항 결과처럼 읽힌다.
    평면이 경제×사회 두 축뿐이라 **안보·제도 문항이면 점을 안 찍는다**
  - `getAxisCloud` 에 **10분 캐시 + inflight** 를 붙였다 (PoliticianService). 홈이 아무리 열려도 269행 쿼리는 10분에 한 번
- 🔴 **B — 큰 문장은 「상태」가 아니라 「결과」를 말한다** (2026-08-25 재작성, 사용자 지적:
  *"그래서 뭐? 이러는 사람도 많을 것 같은데"*). 구 문안 `발의된 법안 100건 중 75건은 **아직 위원회에 있습니다**` 는
  ① 바로 위 숫자를 한 번 더 읽어준 것뿐이고 ② `아직` 이 **곧 처리될 것**처럼 읽혀 사실과 멀어졌다.
  지금은 세 박자다 — **뜻**(`법안을 냈다고 심사되는 건 아닙니다`) → **시간**(그렇게 멈춰 있는 7,501건은
  발의된 지 1년이 넘었다) → **끝**(임기가 끝나면 심사받지 못한 법안은 폐기된다).
  같은 이유로 ②도 고쳤다: `표결 147번에 한 번꼴로 반대표가 나옵니다`(눈금 그림이 이미 하는 말) →
  **`본회의까지 온 법안은 거의 그대로 통과합니다`**, 147 은 각주로 내렸다
  - 🔴 **`.nums-note` 한 줄을 지우지 말 것** (`계류가 곧 방치는 아닙니다…`). 없으면 위 문장이
    **"국회가 일을 안 한다"** 로 읽힌다 — 「월별 추이」에 처리 지연 각주를 반드시 붙이는 것과 **같은 이유**다.
    `.nums-main` 이 `<a>` 라 중첩 링크가 안 되므로 이 줄은 상자 **밖**에 있다 (해설 2편으로 링크)
  - ⚠️ 재료는 `getHomeFacts` 에 컬럼 하나(`bill_pending_over_1y`)를 더한 것이다. 화면에 숫자를 박지 말 것.
    실측(2026-08-25) 계류 **14,079건 중 7,501건(53%)이 1년 초과** · 중앙값 **398일** · 최장 **817일** ·
    위원회 안건으로 아직 상정 안 된 것 4,328건
  - ⚠️ **임기 만료 연도를 화면에 쓰지 않았다.** 22대는 2028년 5월이지만 `ASSEMBLY_AGE` 가 바뀌면 조용히 거짓이 된다.
    시간 압박은 데이터에서 나온 `1년이 넘었고` 가 대신 말한다
- 🔴 **B — 숫자를 키우기만 하지 않는다. 숫자를 그림으로 만든다.** 100칸 그리드(계류 칸 수 = 비율) ·
  눈금 147개 중 하나(반대표 0.68% = 표결 147번에 한 번) · 중앙값 마커. 읽지 않고도 크기가 오는 게 목적이라 장식이 아니다.
  다크 밴드(`.nums-thesis`)는 스크롤 전체에서 **유일하게 어두운 면**이고, A(당신)와 C(오늘)를 잇는 논지가 여기 있다
  - ⚠️ 파생값은 전부 `f`(getHomeFacts)에서 계산한다. `_ticks` 는 **40~200 으로 조인다** — 40 미만이면
    희소해 보이지 않고 200 을 넘으면 한 칸이 1px 미만이 된다
  - ⚠️ 마크업이 `<a>` 안이라 자식이 전부 `<span>` 이다. `.nums-said`·`.nums-sub`·`.say`·`.fn`·`.nums-bar` 에
    **`display:block` 을 빼면** max-width·margin·height 가 통째로 무시된다 (인라인 상자엔 안 먹는다)
  - ⚠️ `<i<%= … ' class="on"' %>>` 처럼 **`<%=` 로 속성을 찍지 말 것** — 따옴표가 이스케이프돼 클래스가 안 붙는다.
    실제로 100칸이 전부 회색으로 나갔다. `<%-` 여야 한다
  - ⚠️ 눈금은 `min-width:1px` 이라 더 못 줄어든다. 320px 안쪽 폭이 **296px** 인데 147개 + 1px 간격이면 293px 로
    여유가 3px 뿐이다 → **≤768px 에서 `gap: 0`**
- 🔴 **B — 세 숫자는 순서가 아니라 「크기」로 위계를 준다** (2026-08-26 디자인 평 반영).
  평: *"세 카드는 첫 카드만 크고 나머지 둘이 나란한데, 서사상 가장 강한 건 20.8%(내 의원이 자리에 없다)다.
  순서나 크기를 바꿀 여지."* → **크기만 바꿨다.**
  - 🔴 **순서(계류 → 반대 → 불참)를 바꾸지 않은 이유**: 셋은 하나의 사슬이고(`getHomeFacts.sql` 머리 주석),
    무엇보다 불참을 맨 앞에 세우면 홈의 첫 주장이 사실상 **"국회의원들이 자리에 없다"** 가 된다.
    장관 겸직·의장 관례·표결 보이콧이 섞인 값이라 평가로 읽히면 안 된다
  - 밴드를 `1fr 1.28fr` 로, 마지막 칸 숫자 44 → **52px** · 문장 15 → **17px**. 뒤로 갈수록 무거워져 다크 밴드로 이어진다.
    ⚠️ ≤768px 은 1열이라 두 칸 폭이 같아진다 → 확대 배율을 되돌린다
  - 🔴 그러면서 **불참 카드에 빠져 있던 각주를 채웠다** (`게으름이 아니라 장관 겸직·의장 관례·표결 보이콧이 섞여 있습니다`).
    SNS 카드(`/promo/numbers`)의 불참 편에는 있는데 홈에는 없었다
- 🔴 **B — 각주는 세 숫자를 「다 보여준 뒤」 한 번에 붙는다** (2026-08-26 이동).
  평: *"단서가 숫자 바로 아래 붙어서 임팩트를 즉시 깎는다."* 맞는 지적이라 **위치만** 옮겼다 —
  카드 1 밑에 있던 시절엔 첫 숫자의 힘을 곧바로 깎고 2·3번으로 가는 리듬도 끊었다.
  🔴 **지우거나 링크 뒤로 접지 않는다.** 없으면 이 섹션이 "국회가 일을 안 한다" 로 읽힌다
  (「월별 추이」에 처리 지연 각주를 반드시 붙이는 것과 같은 이유). 지금은 밴드 아래 가는 선 하나로 층을 갈라 둔다
  - ⚠️ 위 카드 각주(`.fn` 11.5px)와 크기가 비슷해 선이 없으면 한 덩어리로 읽힌다.
    하나는 **그 카드의 각주**, 이건 **섹션 전체의 각주**다
- 🔴 **C — 날짜·출처를 마스트헤드에 다시 쓰지 말 것** (2026-08-26).
  `.hero-strip` 이 `2026년 8월 26일 수요일 · 22대 국회 · 국회 공개 자료 매일 갱신` 을 이미 말하는데
  브리핑 마스트헤드가 같은 문장을 반복해 리듬이 끊겼다 (디자인 평). 신문도 마스트헤드는 한 번뿐이다.
  지금 `.bf-mast` 는 `오늘의 1면` + `전체 보기 →` 뿐이고, 날짜는 리드의 스탬프(`AI 브리핑 · 08.21`)가 갖는다
  - ⚠️ `지난 브리핑` 이라는 말도 한 화면에 두 번이었다 (마스트헤드 링크 + 사이드바 머리) → 링크를 `전체 보기` 로.
    섹션 헤더의 `지난 브리핑 →` 링크도 같이 뺐다 (같은 목적지가 두 줄 위에 또 있었다)
- 🔴 **내 지역구는 「다크 밴드 바로 뒤 · 브리핑 앞」이다** (2026-08-26 이동).
  평: *"홈의 가장 개인적인 후크인데 페이지 하단에 있다."* 맞고, **이 개편이 만든 회귀**다 —
  2026-08-24 배치에서는 브리핑 다음(세 번째)이었는데 B·C 가 들어오면서 밀렸다.
  지금 순서라야 다크 밴드의 `그래서 당이 아니라 사람을 봅니다` 가 곧바로 **내가 뽑은 사람**으로 이어진다
  - 🔴 **히어로로 더 올리지 말 것.** 히어로의 일은 진단 첫 문항이고, 두 개의 첫 행동을 같이 두면 둘 다 죽는다.
    그리고 다크 밴드의 `내 지역구 의원 보기` 가 `#mine` 으로 여기를 가리키므로 위로 올리면 링크가 **뒤를** 가리킨다
  - ⚠️ "등록자에겐 홈 상단이 내 의원으로 시작" 안은 **보류**했다. 실측 지역구 등록자가 **1명**(가입 17 · 활성 10)이라
    아직 그 상태를 위해 페이지를 재구성할 근거가 없다. 지금 문제는 표시가 아니라 **등록 진입로의 깊이**였고 그건 이 이동으로 풀린다
- 🔴 **C — 브리핑을 리드 기사 크기로.** 마스트헤드 괘선 + `AI 브리핑 · 08.21` 스탬프 + 세리프 900 37px 헤드라인 +
  **2단 조판 + 드롭캡**, 오른쪽은 지난 브리핑 목록. 매일 바뀌는 유일한 콘텐츠라 **재방문의 이유가 여기 있다**
  - 🔴 **사이드바에 「오늘의 숫자」를 다시 두지 말 것** — 바로 위 B 가 그 셋을 크게 말한다
  - ⚠️ `column-count` 안에서는 `-webkit-line-clamp` 가 **안 먹는다** (브라우저가 `-webkit-box` 를 `flow-root` 로 바꾼다).
    데스크톱은 서버가 340자로 자르고(문장 경계까지), 모바일은 `column-count: auto` 로 되돌린 뒤 5줄 클램프
  - 🔴 **헤드라인은 `noBreakDot()` 을 거친다.** `야간노동 보호·농협 개혁·폐교 활용…` 이 `개혁 / ·폐교` 로 끊겨
    둘째 줄이 점으로 시작했다. `word-break: keep-all` 은 한글끼리만 막아 `·` 앞뒤엔 안 먹으므로
    **U+2060(word joiner)** 로 그 자리만 붙인다. 폭 0인 비가시 문자라 **제목 표시에만** 쓸 것 (복사하면 따라간다)
- 🔴 **미완료자에게 「나와 맞는 의원」 섹션을 그리지 않는다** — 구 `.match-cta`("성향 진단 시작하기")가
  히어로와 **같은 행동을 두 번 조르는** 꼴이 된다. `.match-cta` CSS 는 남겨 뒀다 (참조 0)
- ⚠️ **결론 3숫자는 이제 B 에 있다.** 「숫자로 본 국회」 머리의 `.facts-strip` 은 마크업만 뺐고
  CSS 는 남겼다 (참조 0). **거기에 다시 그리지 말 것** — 한 페이지에서 같은 숫자를 두 번 말하게 된다
- ⚠️ **히어로 지도는 1열이 되면 폭을 조여야 한다.** 판이 거의 정사각(520×460)이라 976px 폭이면
  **863px 높이**가 된다 (1024px 실측). `≤940px` 에서만 접고 `max-width: 620px`
  - 접는 폭이 **1024 가 아니라 940** 인 이유: 1024 에서 접으면 히어로가 1,195px 인데 2열로 두면 **649px** 이다
- ⚠️ **SVG 글자는 viewBox 와 함께 줄어든다.** 520 유저단위 판이 375px 화면에서 301px 로 그려지면 배율 0.579 라
  `font-size: 11` 이 **6.4px** 이 된다 (`/xray` 차트에서 2.6px 까지 뭉개졌던 것과 같은 문제).
  ≤768px 에서 `.hero-map svg text { font-size: 18px }` 로 화면 **10.4px** 을 맞췄다. 값을 내리기 전에 실제 렌더 크기를 잴 것
  - ⚠️ 세로축 라벨 `x` 는 **22** 다. 13 으로 두면 회전된 글자 상자가 SVG 왼쪽 밖으로 나가 잘린다
- ⚠️ 히어로 문구에 **"오른쪽 점"** 처럼 방향을 쓰지 말 것 — 모바일에선 지도가 **위**로 간다
- **실측 (2026-08-25)**: 데스크톱 1440 문서 2,710 → **3,501px** · 모바일 375 3,486 → **4,839px**.
  길어진 건 사실이고 의도다 (히어로 375→784px, B 726px 신규). 대신 **첫 화면의 주 행동이 하나**로 줄었다
  (구 미등록 첫 화면은 행동 요소 8개). 320·375·768·939·1024·1280·1440px 가로 오버플로 **0**,
  전 페이지 500 **0건**, 미완료·부분·완료 세 상태 렌더 확인

#### 결론 3숫자 — 2026-08-25 부터 **B 섹션(「국회는 이렇게 굴러갑니다」)** 이다 (`getHomeFacts.sql`)
> ⚠️ 구 위치(「숫자로 본 국회」 머리의 `.facts-strip`)에 되돌리지 말 것. 아래 순서·주의는 그대로 유효하다.
...
법안 상세 댓글이 **같은 행동을 두 곳에서 받고** 있었고, 둘 다 0건이었다.

🔴 **그런데 근본 원인은 경쟁이 아니라 "대화가 보이지 않는다" 였다.**
댓글 네 종류(`bill`·`politician`·`briefing`·`post`)가 `comments` 한 테이블에 다 들어가는데
**모아 보는 화면이 없었다** (`getRecentComments` 같은 쿼리 자체가 없었다).
법안 댓글이 18,741곳에 흩어져 달아도 아무도 못 보니, 사람들은 목록이 있는 자유게시판으로 몰린다 →
**"기록 옆의 대화" 를 죽이고 "맥락 없는 잡담" 을 살리는 구조**였다. 원래 의도와 정반대다.

| | 법안·의원·브리핑 댓글 | `/community` 글 |
|---|---|---|
| 무엇 | **대상에 대한 반응** | 대상이 없거나 여러 건을 엮는 이야기 |
| 주제 | 고정 (이 법안) | 없음 → 진영 일반론으로 흐른다 |
| 사실 | **바로 옆에 있다** (표결 기록이 즉시 검증) | 없다 |
| 집중 | 18,741곳으로 분산 → 점령이 어렵다 | 한 곳 → 점령당한다 |

**① 법안 상세 「이 법안을 다룬 글」** (`.rp`, `daos/queries/post/listByBillId.sql`)
- 링크가 **한 방향뿐이었다** — 글은 법안을 첨부할 수 있는데(`posts.linked_bill_id`) 법안 쪽에서 그 글을 볼 수 없었다
- 🔴 **국민 의견 섹션 *안*, 댓글 위젯 *위*가 제자리다.** 새 섹션으로 빼면 `.bd-toc` 탭이 하나 느는데
  대부분의 법안에서 비어 있어 **도달 불가 탭**이 된다. 관련 글도 댓글과 같은 "사람들의 말" 이다
- 🔴 **글이 없으면 통째로 렌더하지 않는다** (빈 상자는 거의 모든 법안에서 죽은 공간)
- ⚠️ 부가 정보라 조회 실패해도 `[]` 로 접는다 — 법안 페이지는 살아야 한다 (AI 분석과 같은 처리)

**② 「최근 대화」 피드** (2026-08-27, `daos/queries/comment/listRecent.sql`)
- ⚠️ **같은 날 ④에 흡수됐다.** 별도 탭(`?tab=talk`)이 아니라 `/community` 기본 화면이 통합 피드가 됐다.
  `listRecent.sql` 은 남아 있지만 **화면에서 쓰지 않는다** (댓글만 모아 보는 화면이 다시 필요해지면 그때)
- 🔴 **정렬은 시간순뿐이다. 인기순·추천순을 넣지 말 것** — 어느 대화를 띄울지가 곧 편집이고,
...
- ⚠️ `comments.target_id` 는 **VARCHAR** 다. briefing·post 는 숫자 PK 라 **`id::text`** 로 캐스팅 —
  반대 방향(`target_id::bigint`)은 쓰레기 값에 통째로 터진다. (`likes.target_id` 는 INTEGER — 섞지 말 것)

**③ 글쓰기 권유** (`views/community/write.ejs` `.wr-bill-hint`)
- 법안을 첨부하면 `법안 페이지에 바로 의견 남기기 ↗` 한 줄이 뜬다. 링크는 그 법안의 `#part-comments`
- 🔴 **막지 않는다. 권유만 한다** — 강제하면 "쓸 곳이 없다" 가 되어 고치려던 문제(대화가 안 생긴다)가 되돌아온다.
  여러 건을 엮거나 배경을 길게 쓸 때는 글이 맞다고 문구가 같이 말한다
- ⚠️ **수정 모드는 첨부 상자가 SSR 로 열린 채 뜬다** — 링크 href 를 JS(`attachBill`)로만 채우면 그때 비어 있다.
  마크업 뒤 인라인 스크립트가 SSR 분기를 메운다

**④ `/community` = 통합 피드** (2026-08-27, `daos/queries/post/listFeed.sql`)
🔴 **글과 댓글이 한 줄기로 흐르는 게 기본 화면이다.** ②까지는 둘이 탭으로 갈라져 있었는데,
그러면 댓글은 여전히 탭 하나 뒤에 숨고 첫 화면은 계속 "대상 없는 글" 만 보여준다 — **고치려던 구조가 그대로 남는다.**

```
?on=all(기본)|bill|politician|briefing|posts   무엇을 보나 (첫 줄 탭)
?type=<글 유형>                                 `on=posts` 일 때만 (둘째 줄 탭)
```
- 🔴 **`posts` 는 커뮤니티 글만이다.** 글에 달린 댓글을 여기 넣으면 탭 숫자(글 3)와 실제 행 수(4)가
  어긋나고, 둘째 줄 글 유형 탭도 댓글엔 걸 수 없어 뜻이 무너진다. 글 댓글은 `all` 에서 보인다
- 🔴 **키를 `post` 로 쓰지 말 것** — 댓글 종류(`comments.type='post'`)와 글자가 겹쳐 어느 쪽인지 안 보인다
- 🔴 **둘째 줄은 첫 줄보다 가벼워야 한다** (`.cm-tabs.is-sub`). 같은 무게면 두 줄이 한 덩어리로 보여
  무엇이 무엇의 하위인지 안 읽힌다
- 🔴 **한 목록에 두 종류가 섞이므로 배지가 그 둘을 가른다** — 글은 유형(공지·잡담…), 댓글은 어디에 달렸는지(법안·의원…)
- 🔴 **고정 공지는 어느 필터에서도 맨 위**(`is_pinned DESC`), 그 밖은 시간순
- ⚠️ **범위 밖 페이지를 "내용 없음" 이라고 하면 거짓말이다** — 내용은 있고 이 쪽이 없을 뿐이다.
  `page > 1` 이면 `이 페이지에는 더 이상 없습니다 · 처음으로 →` 를 쓴다
- ⚠️ 구 링크 호환: `?type=` 만 있으면 `on=posts` 로 친다. `?tab=talk` 는 무시되고 기본(전체)으로 접힌다
- ⚠️ `listFeed.sql` 은 posts·comments 를 **둘 다** 읽는다. `queries/post` 에 둔 건 커뮤니티 페이지를
  PostController 가 그리기 때문이지 posts 만의 것이라서가 아니다
- ⚠️ **nav 이름은 `커뮤니티` 를 유지한다.** `대화` 로 바꾸면 무슨 페이지인지 덜 분명해지고,
  이 프로젝트가 지향하는 것도 "정치 커뮤니티" 다 (페이지 자체가 피드임을 설명한다)
- ⚠️ `COUNT(*) OVER()` 는 조건에 맞는 행을 전부 훑는다 (`post/list.sql` 과 같은 패턴).
  수만 건이 되면 커서 페이징으로 바꿀 것

**착지점 앵커 — 피드가 여기로 사람을 보내므로 셋이 맞아야 한다**
| 종류 | 링크 |
|---|---|
| 법안 | `/bill/:id#part-comments` |
| 의원 | `/politician/:id#comments` — 🔴 **요소 id 가 아니라 JS 가 탭을 여는 방식**이다 |
| 브리핑 | `/briefing/:id#part-comments` (2026-08-27 앵커 신설) |
| 글 | `/community/:id` |
- 🔴 **의원 상세의 `#comments` 는 탭만 열고 스크롤을 안 했다.** 분석 탭이 기본이라 화면은 맨 위에 그대로 있고,
  모바일 문서가 7,000px 대라 정작 보러 온 댓글이 한참 아래 남았다 → `scrollIntoView` 보강.
  ⚠️ 같은 파일의 `data-open-tab` 경로와 **같은 동작**이어야 한다 (한쪽만 고치면 같은 탭인데 다르게 도착한다)

**네 단계 모두 2026-08-27 에 끝났다.** 선결 조건이던 신고 처리 화면도 같은 날 만들었다 (아래 「신고 처리」).

⚠️ **아직 안 한 것**: 대화가 실제로 쌓인 뒤에야 판단할 수 있는 것들 — 알림(내 글에 답글이 달렸다) ·
차단(특정 사용자) · 정렬 선택지. 지금 만들면 전부 추측이다

✅ 실측 2026-08-27: 글 2 + 댓글 4종 + 대량 22건으로 검증 —
필터 5종 행·배지·링크 정확 · **「글」 탭 숫자와 행 수 일치**(3=3) · 유형 하위탭 6종 ·
페이징이 필터를 유지(전체 20+9 · 법안 20+3) · 범위 밖 안내 · 구 링크(`?type=`·`?tab=talk`) 폴백 ·
...
처리 상태를 기록할 컬럼도 없었다 — **있는 줄 알았는데 없는 기능**. 「최근 대화」 피드로 댓글이 보이기
시작하면 곧 필요해지고, 선거법상 후보자 비방·허위사실 삭제 의무와도 직결된다 (ELECTION_LAW.md §5).

```
utils/reportReasons.js       ← 사유·종류·상태 단일 소스
POST /api/reports            ← 접수 (requireLogin) · GET /api/reports/reasons
/admin/reports?status=        ← open(기본) | handled | all
POST /admin/reports/:type/:targetId  ← action=keep|remove
ddl/migrations/2026-08-27-reports-status.sql
```

- 🔴 **처리 단위는 「신고」가 아니라 「대상」이다.** 관리자가 판단하는 건 "이 댓글을 지울까" 이지
  "이 신고를 처리할까" 가 아니다. 같은 댓글에 신고 3건이면 **한 줄로 뜨고 한 번의 판단으로 3건이 같이 닫힌다**.
  `status` 는 사실상 **대상의 상태를 각 행에 복제해 둔 것**이라 처리 시 그 대상의 **모든** 행을 갱신한다
- 🔴 **`political` 사유를 화면에 되살리지 말 것.** DB CHECK 에는 있지만 `reportReasons.js` 에서 뺐다 —
  "정치적" 을 신고 사유로 열면 곧 진영 신고 도구가 되고, 중립을 표방하는 사이트가 그 이유로 글을 지우면
  그 자체가 편집이다. 선거법이 요구하는 건 `abuse`·`false_info` 가 이미 덮는다. **서버가 400 으로 막는다**
- 🔴 **「살려둠」이 대상을 되살리는 것은 우리가 지운 경우(`status='removed'`)뿐이다.**
  작성자 본인이 지운 글까지 되살리면 삭제 의사를 뒤집는 것이라 하면 안 된다 (실측 확인)
- 🔴 **접수 결과에 신고 건수를 보여주지 않는다** — 몇 명이 신고했는지가 보이면 그게 곧 여론 표시가 되고
  동조 신고를 부른다. 건수는 관리자만 본다
- 🔴 **미처리 수는 `AdminRoutes` 미들웨어가 전 관리자 화면에 주입한다** (`res.locals.openReportCount`).
  신고는 주기 작업이 아니라 **사건**이라 "가끔 들러서 확인" 으로는 잊힌다. 페이지마다 따로 세면 갈린다.
  ⚠️ 실패해도 화면을 죽이지 않는다 (null 이면 숫자를 안 그린다)
- ⚠️ **HAVING 으로 필터한다** (WHERE 아님). WHERE 로 걸면 그룹 안에 그 상태의 행만 남아 건수가
  대상 전체를 못 나타내고, 「살려둠 2건 + 새 신고 1건」 인 대상이 양쪽 목록에 다르게 보인다
- ⚠️ **재신고하면 `status` 가 `open` 으로 돌아간다** — 처리 뒤에 또 신고가 들어왔다는 신호다 (실측: 처리됨 → 미처리 1)
- ⚠️ **대상 실재를 서버가 확인한다.** `reports` 는 대상이 두 테이블이라 FK 가 없어, 없는 id 로도 행이 생기고
  관리자 화면에 내용 없는 유령 행이 남는다. 반대로 **이미 사라진 대상의 신고는 숨기지 않는다** (목록 수가 안 맞아 보인다)
- ⚠️ `reports.target_id` 를 **INTEGER → BIGINT** 로 넓혔다 (`comments.id`·`posts.id` 가 BIGINT).
  조용히 오버플로하는 종류였다. `likes.target_id` 는 아직 INTEGER 다
- ⚠️ 관리자 삭제는 **본인 확인이 없는 별도 쿼리**다 (`admin/setCommentDeleted.sql`). 일반 경로(`comment/softDelete.sql`)와 섞지 말 것
- 접수 UI: 댓글 위젯(`interactions.js` — 법안·의원·브리핑·글 공용) + 커뮤니티 글 상세.
  **내 글·삭제된 글엔 안 뜨고, 비로그인에도 안 뜬다** (눌렀을 때 로그인으로 튕기면 무슨 일인지 알 수 없다).
  🔴 사유 목록을 화면에 하드코딩하지 말 것 — `/api/reports/reasons` 가 단일 소스를 그대로 내려준다
- ✅ 실측 2026-08-27: 접수 8케이스(비로그인 401 · 정상 · 재신고 갱신 · **political 400** · 모르는 사유/종류 400 · 없는 대상 404) ·
  권한 3종(비로그인·일반 **404** / 관리자 200) · 처리 3종(살려둠 · 삭제 → `is_deleted` · 되살리기) ·
  **작성자 삭제분은 되살아나지 않음** · 잘못된 입력 4종 PRG · **Origin 위조 403** · 배지 4페이지 · 필터 폴백 ·
  800px 가로 오버플로 0. 테스트 데이터·세션 전량 삭제 확인
...
- ⚠️ **`balance` 는 `/balance-game/*` 전체가 한 행으로 합쳐진다** (비캡처 규칙이라 target_id 가 빈 값). 초대 화면 열람과 문항 시작을 **구분하지 못하고**, 홈 히어로에서 시작한 사람은 `home` 으로 잡힌다. SNS 지표로 쓸 때 "진단 시작" 이라고 부르지 말 것 (SNS.md 0장)
- ⚠️ **`/promo/*` 는 세지 않는다** (2026-08-26 추가). robots 의 `Disallow: /promo/` 와 짝인데 `SKIP_PATH` 에만 빠져 있었다 — 운영자가 로그아웃 상태(폰에서 PNG 저장)로 열면 `other` 로 잡혀 비회원 방문이 부푼다
...
- 🔴 **회원/비회원을 나눠 센다** (2026-08-18). 운영 초기엔 **관리자 본인 브라우징이 통계를 지배한다** —
  실측 08-16 전체 192뷰 중 회원 154뷰(80%), 그중 관리자 1명이 131뷰. "진짜 외부 사람" 을 볼 수가 없었다.
  - `page_views_daily.member_views` / `member_uniques` 를 **같은 행에 더했다**. 비회원 = `views - member_views`
  - 🔴 **PK 를 쪼개지 않았다** — `is_member` 를 PK 에 넣으면 기존 행의 의미가 갈리고 읽기 쿼리 5종을 전부 고쳐야 한다.
    컬럼을 더하면 `views`(전체)의 뜻이 유지돼 과거 데이터·기존 쿼리가 안 깨진다
  - 🔴 **관리자(`ADMIN_EMAILS`)는 아예 세지 않는다.** 제외해야 회원 지표도 "진짜 사용자" 를 가리킨다
  - ⚠️ **2026-08-18 이전 행은 `member_*` 가 0** 이라 그 구간 비회원이 과대계상된다. 화면이 각주로 밝힌다 (소급 보정 불가)
  - ⚠️ 관리자·회원이 **로그아웃 상태로 보면 비회원으로 잡힌다** — 쿠키만으로는 사람을 알 수 없다
  - ⚠️ `member_uniques` 는 로그인 상태별로 따로 센다 — 같은 사람이 하루에 두 상태로 보면 양쪽에 1씩 (근사값)
  - ⚠️ **목록형 `PAGE_KINDS` 규칙에 캡처 그룹을 쓰지 말 것** — `(/|$)` 가 구분자를 잡아 `target_id` 가 `'/'` 가 되고,
    같은 종류가 `''`·`'/'` 두 행으로 갈린다 (실측 balance 11 + 14). 비캡처 `(?:…)` 로 (2026-08-18 수정)
- 🔴 **신규/재방문을 나눠 센다** (2026-08-27, `ddl/migrations/2026-08-27-page-views-returning.sql`).
  운영 초기에 **유일하게 의미 있는 지표가 재방문**이다 — 유입은 SNS 가 만들어주지만 재방문은 콘텐츠가 만든다.
  가입 수는 후행 지표다 (진단·지역구를 비로그인으로 열어둬서 **가입할 이유를 일부러 없앴다**).
  - 🔴 **잴 수단이 없었다.** `_v` 쿠키는 1년짜리로 잘 발급되고 있었는데 서버는 그걸 **그날 안에서 중복 제거하는
    용도로만** 썼다 (`state.seen` 은 프로세스 메모리 Set, 매일 리셋). "이 사람이 어제도 왔나" 를 알 방법이 없었다
  - 🔴 **쿠키 값에 최초 방문일을 담아 판정한다** — `_v` = `<16hex>.<YYYYMMDD>`. 서버는 그 날짜를 **저장하지 않고
    오늘과 비교만** 한다 → 방문자별 기록 없이 일별 합계만 남는 원칙 그대로. `new_visitors`/`returning_visitors` 두 컬럼
  - 🔴 **`page_kind='site'` 행에만 값이 들어간다.** 페이지마다 세면 한 방문자가 본 페이지 수만큼 중복돼
    합계가 방문자 수를 넘는다. `bumpVisitor()` 가 `visitor|<vid>` 키로 하루 한 번만 센다
  - 🔴 **`site` 행에서 신규 + 재방문 = `uniques`** 다 (실측 2026-08-27: 4 = 2 + 2). 같은 집합을 쪼갠 값이라
    **어긋나면 집계 버그다** — 점검 신호로 쓸 것. ⚠️ 회원/비회원 분해와는 **축이 다르다** (신규·재방문은 회원 여부를
    나누지 않는다 — 쿠키는 사람을 모른다). 두 분해를 교차해 더하지 말 것
  - ⚠️ **구 형식 쿠키(날짜 없음)는 재방문으로 센다** — 쿠키가 있다는 것 자체가 전에 왔다는 뜻이다. 다만 최초 방문일을
    모르므로 오늘 날짜로 다시 발급한다 → **도입 직후 며칠만 재방문이 과소 계상**되고 그다음부터 정상
  - ⚠️ **소급이 안 된다.** 2026-08-27 이전 행은 둘 다 0 이고 그 구간은 영영 못 잰다. 화면이 각주로 밝힌다
  - ⚠️ 다른 기기·시크릿창·쿠키 삭제는 새 방문자로 잡혀 **재방문이 실제보다 낮게** 나온다
  - 🔴 **개인정보처리방침 6항을 같이 고쳤다** — 구 문구가 `무작위 값` · `하루 단위 방문자 수를 세는 데만` 이라
    날짜를 더한 순간 거짓이 됐다. **쿠키 이름·기간뿐 아니라 담는 값이 바뀌어도 거기를 볼 것**
  - ✅ 실측 2026-08-27 (`Host` 헤더로 `LOCAL_HOST` 우회 · 6 시나리오): 첫 방문 → 신규 · 같은 쿠키 재요청 → 중복 없음 ·
    어제 날짜 쿠키 → 재방문(재발급 0) · 구 형식 → 재방문 + 날짜 붙여 재발급(hex 유지) · 깨진 값 → 새 발급 + 신규 ·
    봇 UA → 미집계. 결과 `site 6뷰/4유니크 · 신규 2 · 재방문 2`. 템플릿은 데이터 있음/빈 상태 둘 다 렌더 확인
...
- **Zone 4 — 섹션 탭 바** (`.bd-toc`, 2026-08-24 전면 교체):
  🔴 **구 버전은 이 페이지만 내비게이션이 두 벌이었다** — 우측 세로 레일(`.pb-section-index`, fixed 180px)
  \+ 모바일 플로팅 바(`.pb-mobile-jumpbar`, 하단 알약). 사이트에 형태가 셋이 되어 있었다 (사용자 지적).
  둘 다 지우고 **사이트 공통 밑줄 탭 하나**로 합쳤다. 되돌리지 말 것.
  - **`/bill`·`/politician` 의 탭과 같은 값** — `padding: 14px 22px` · `14px/500` · 활성 `--text` +
    `border-bottom: 2px` 골드 / 컨테이너 `padding: 0 24px` · 흰 배경 · `border-bottom: 1px` · gap 없음.
    `/issue/:slug` 의 `.iss-toc` 와도 같다
  - 🔴 **`.bd-wrap` 바깥의 형제다. `100vw` 로 breakout 하지 말 것** — `100vw` 는 스크롤바를 포함해
    **문서에 가로 오버플로가 생긴다.** 형제로 두면 `width: auto` 가 곧 클라이언트 폭이라 계산도 오버플로도 없다
  - 🔴 **흰 배경·보더는 `-inner`(960px) 에 준다** — 사이트 공통 규칙 그대로다. 바깥에 주면
    흰 띠가 화면 끝까지 뻗어 본문과 따로 논다 (`/issue` 에서 한 번, 여기서 또 한 번 같은 실수를 했다)
  - 🔴 **다만 바깥(`.bd-toc`)은 투명이 아니라 `--bg`(페이지 배경) 로 채운다. 이 페이지에만 있는 사정이다** —
    Zone 3 마진노트(`.ba-margin-note`)가 본문 왼쪽 **바깥**에 있어서(실측 1440px 에서 150~251px ·
    본문은 275px 부터) 바깥이 투명이면 스크롤할 때 그 칩이 흰 띠 옆으로 지나간다.
    `--bg` 로 채우면 **눈에는 페이지 배경 그대로**라 띠처럼 안 보이면서 가려주기만 한다.
    `/bill`·`/politician`·`/issue` 는 콘텐츠가 전부 띠 안에 들어와 투명으로 충분하다
  - 🔴 **breadcrumb 는 탭 바 위**(`.bd-top`)다. 아래에 두면 sticky 바 밑에 깔린다.
    고정 nav 클리어런스도 `.bd-top` 이 갖는다 — `.bd-wrap` 의 `padding-top` 에 다시 넣지 말 것
  - 🔴 **탭의 보이는 순서를 JS 가 실제 DOM 위치로 정렬한다.** 세로 레일 때는 카테고리 묶음이라 조금 어긋나도
    읽혔지만, 가로 바에서는 왼→오가 곧 읽는 순서다 (`#bill-timeline` 이 분석 분기에서 Zone 1 뒤로 옮겨진다)
  - 클릭 오프셋 = `--nav-h` + **탭 바 높이** + 12. ⚠️ 바 높이를 상수로 박지 말 것 (`offsetHeight` 로 읽는다)
  - 데이터 카운트(찬반·의견)는 로드 후 `PB.fetch` 로 채운다 (본회의 표결은 서버 SSR)
  - **스크롤 기반 활성 트리거** (IntersectionObserver 의 좁은 활성 띠로 짧은 마지막 섹션 못 잡는 문제 회피).
    배열 순서대로 순회 → `top <= offset` 인 마지막 섹션 active. 페이지 끝에서는 마지막 강제 활성
  - 실측 2026-08-24 (375·768·1024·1440·1920px): 바 높이 **56px** · 흰 띠 = `.bd-wrap` 폭(960)과 일치 ·
    바깥 배경 = body 배경과 동일 · breadcrumb=탭=참여카드 좌측 기준선 일치 ·
    탭 10개 960px 안에 한 줄(모바일 가로 스크롤) · 마진노트 가려짐 · 도달 불가 0 · 클릭 불일치 0
  - ⚠️ **분석 있는 법안은 가로 오버플로가 5px 남는다.** 원인은 `.ba-shell` 의 `width: 100vw` 로
    **이 교체와 무관한 기존 값**이다 (미분석 분기는 0). 고치려면 본문 폭 계산을 다시 잡아야 한다
...
- ⚠️ Zone 1~5 는 `PB.mountBillAnalysis` 가 렌더해서 EJS 가 Zone 1↔2 사이에 끼어들 수 없다. mount 직후 `insertAdjacentElement('afterend')` 로 옮긴다. JS 실패 시 SSR 위치에 남아 정보는 유지된다
  - ⚠️ 이 이동 때문에 **탭 바의 마크업 순서와 실제 DOM 순서가 어긋난다** → `.bd-toc` 가 실제 위치로 다시 정렬한다
...
- 섹션 탭 바(`.bd-toc`)에 `원문` 탭으로 들어간다 (2026-08-24. 구 버전은 모바일 jumpbar 4탭을 유지하려고 sticky 인덱스에만 뒀는데, 지금은 바가 하나라 그 구분이 없다)
...
- 섹션 탭 바에 `제안이유` 탭 추가 (분석 없고 `bill.summary` 있을 때만)
...
### `/bill/:id` 「이 페이지가 말하지 않는 것」 `#bill-caveat` (2026-08-24)
공용 컴포넌트 `.pb-caveat` (main.css). **페이지의 맨 끝**, 국민 의견 다음이다.

- 🔴 **국민 의견 뒤에 두는 이유** — 이 상자가 "우리가 마지막으로 하는 말" 이기 때문이다.
  그 아래는 다른 사람들의 말이다. `/guide`·`/issue` 도 같은 자리(맨 끝)라 셋이 한 패턴으로 읽힌다
- 🔴 **항목은 이 페이지의 데이터에만 붙인다** — 일반론을 늘어놓으면 안 읽힌다. 조건부로 나온다:
  | 항목 | 조건 |
  |---|---|
  | AI 분석은 사람이 검수한 글이 아니다 (AI 가 읽은 문서 ≠ 국회 원문) | `analysis` |
  | 찬성이 많다고 이견이 없었다는 뜻이 아니다 (걸러지는 곳은 위원회) | `hasVotes` |
  | 처리 경과는 날짜만 안다 (위원회 표결은 개인별 비공개) | `tlHasAny` |
  | 공동발의자 수는 기여도가 아니다 | 공동발의 있음 |
  | 국민 찬반은 여론조사가 아니다 (표본이 치우친다) | 항상 |
  실측: 분석+표결 **5개** · 분석+계류 4개 · 미분석+계류 **3개**
- 🔴 **숫자를 본문에 박지 말 것.** 배치가 매일 움직여 조용히 거짓이 된다 —
  대신 「읽는 법」으로 링크한다 (`how-far-to-trust-ai`·`why-so-few-no-votes`·`how-a-bill-moves`·
  `proposing-is-not-contribution`). 부수효과로 각주가 해설로 가는 허브가 된다
- 섹션 탭 바(`.bd-toc`)의 마지막 탭이 `한계` 다
- ⚠️ sticky 인덱스에서는 **참여 그룹 뒤 · 「한계」 그룹**이다. 실측 항목 높이 31px 로 나머지와 같고 한 줄에 들어간다

#### 🔴 sticky 인덱스 활성 추적은 **실제 DOM 위치로 정렬**한다 (2026-08-24)
`idxTargetIds` 를 마크업 순서 그대로 쓰면 안 된다. 활성 판정이 "배열에서 마지막으로 걸린 것" 을
고르므로, **배열이 DOM 순서가 아니면 뒤에 적힌 항목이 앞 섹션을 통째로 가린다.**

- 원인: `#bill-timeline` 은 분석 분기에서 **JS 가 렌더 후 `.ba-z1` 뒤로 옮긴다** (그게 제자리다).
  그래서 인덱스 마크업(6번째)과 실제 DOM(2번째)이 어긋난다. 미분석 분기도 같은 이유로 어긋나 있었다
- 실측 (1440×900 · 분석 있는 법안): **핵심 변화·분석·함께 생각·원문 대조 4개가 도달 불가**였고,
  눌러도 「처리 경과」 가 켜졌다. 처리 경과 혼자 스크롤 구간의 **64%(2,084px)** 를 먹고 있었다
- → `setupIndexObserver` 안에서 `compareDocumentPosition` 으로 정렬. 정렬 후 도달 불가 0 · 클릭 불일치 0
- ⚠️ **반드시 `ba-summary` 대기 뒤에 정렬할 것** — 그 전엔 5-Zone 과 타임라인이 아직 제자리에 없다
- ⚠️ 인덱스 nav 의 **보이는 순서**는 여전히 카테고리 묶음(AI 분석 · 경과 · 참여 · 한계)이라
  스크롤 순서와 완전히 같지는 않다 (경과 칩이 아래 있는데 섹션은 위). 활성 표시는 이제 정확하다
...
### `/issue` 쟁점 — 뉴스를 다루되 **기사를 싣지 않는다** (2026-08-23)
정치 뉴스는 유입의 핵심인데 기사를 고르는 순간 편집이 된다. 그래서 뒤집었다 —
**뉴스는 진입 키워드로만 쓰고, 화면에 나가는 건 전부 DB(공적 기록)다.**
뉴스는 "누가 뭐라고 했나" 를 말하고, 이 페이지는 "**국회 기록에 뭐가 남았나**" 를 말한다.

```
utils/issues.js              ← 쟁점 정의 단일 소스 (slug·title·keywords·why)
daos/queries/issue/*.sql     ← getIssueBills · getIssueSigners (WHERE 는 DAO 가 조립)
daos/IssueDao.js             ← 키워드 → 자리표시자 조립 + 파라미터 바인딩
services/IssueService.js     ← 근거 법률별 묶음 + 집계 + 10분 캐시·inflight 공유
controllers/IssueController.js · views/issue/*.ejs · public/styles/issue.css
```

- 🔴 **편향은 네 곳에서 들어온다. 각각 방어가 있다:**
  | | 방어 |
  |---|---|
  | ① 무엇을 다룰지 고르기 | **법안 활동이 이미 있는 것만.** 뉴스가 아무리 시끄러워도 발의가 없으면 안 만든다 → "우리가 골랐다" 가 아니라 "국회가 이미 다루고 있다" 가 기준. 그 근거(`why`)를 **화면에 그대로 노출**한다 |
  | ② 뭐라고 부를지 | 제목은 **법률명·제도명으로만**. 사건명·인물명·진영 프레임 금지 (`OO 게이트` ✗ / `법원행정처와 사법행정 구조` ✓) |
  | ③ 어떤 지표를 보여줄지 | 쟁점마다 새 지표를 만들지 않는다 (뷰가 고정 세트만 쓴다) |
  | ④ 어떤 매체를 인용할지 | 인용하지 않는다. **검색 링크만** (브리핑과 같은 규칙) |
- 🔴 **자동 판별은 안 된다 — 순위를 매기지 말 것.** 신호 셋을 전부 실측했고 전부 실패했다 (`법원행정처` 28건 기준):
  | 신호 | 실패 방식 |
  |---|---|
  | 등장 빈도 | 국제민사사법공조법이 6회(송달 경유지)로 1등, 정작 **폐지를 제안한 전현희 안은 2회**라 탈락 |
  | 첫 등장 위치 | 오탐은 밀렸지만 부수 언급(박범계 대법관증원안)이 8%로 최상위에 남음 |
  | 방향 동사 근접 | `폐지` 는 정확히 잡았으나 `설치`·`신설` 이 법률명에도 흔해 무관한 법률이 딸려옴 |
  → 신뢰할 수 있는 자동 분류는 **근거 법률(`bill_name`) 하나뿐**이다. 그걸로만 묶고 노이즈는 사용자가 눈으로 거른다.
  키워드 매칭의 역할은 **18,830건 → 10~100건으로 좁히는 것**까지다 (판별은 5-Zone 해설이 맡는다)
- 🔴 **`.iss-caveat` 4줄을 지우지 말 것.** 없으면 이 목록이 "검증된 쟁점 법안" 으로 읽힌다. 특히
  **"반대하는 쪽의 기록은 구조적으로 적다"** — 제도를 바꾸자는 사람은 법안을 내지만 지금대로 두자는 사람은 낼 이유가 없다
  (`institution 안정` 셀이 얇은 것과 **같은 구조**다). 이걸 안 밝히면 법안 수가 곧 여론으로 읽힌다
- ⚠️ **키워드는 3글자 이상.** 2글자는 trigram 인덱스가 안 만들어져 Seq Scan 이 되고(실측 `국방` 902ms) 오탐도 폭증한다.
  DAO 가 3글자 미만을 버린다. 인물명을 키워드로 쓰지 말 것 — 그 자체가 편집이다
- ⚠️ **`ILIKE ANY(배열)` 을 쓰지 말 것** — trigram 을 타긴 하는데 **32.3ms vs 1.9ms** (17배). DAO 가 명시적 OR 로 조립한다.
  조립되는 건 자리표시자뿐이고 값은 전부 바인딩 (ChartDao 와 같은 원칙)
- ⚠️ 단건 법률을 **버리지 않는다** — "한 건씩 있는 법률" 로 묶어 전부 보여준다. 숨기면 머리의 총 건수와 목록 합계가 안 맞아 "나머지는 어디 갔나" 가 된다
- ⚠️ 법안 건수(대표발의)와 의원 수(고유 서명자)는 **세는 단위가 다르다.** 화면이 각주로 밝힌다
- 🔴 **「지금까지 어떻게 됐나」(`.iss-outcome`)가 이 페이지의 답이다.** 처음엔 처리 상태를 칩(`계류 61 · 대안반영폐기 38`)으로만
  뿌렸는데, 사용자 지적이 정확했다 — **"전세사기 보러 들어왔는데 그래서 뭐 어떻다는 거야?"** 칩은 그 질문에 답하지 않는다.
  `proc_result_name` 을 **네 갈래 서사**로 접는다 (통과 / 위원회가 합침 / 발의자가 거둠 / 아직 심사 중) + 통과한 법안은 **목록으로 링크**.
  실측 전세사기: `1건 통과(주택도시기금법, 2025.11.13) · 38건 합쳐짐 · 2건 철회 · 61건 심사 중(가장 오래 787일째)`
  - ⚠️ 모르는 결과명은 **버리지 말고 `other` 로 모은다** (`OUTCOME` 매핑 밖). 버리면 갈래 합계가 총 건수와 안 맞아 "나머지는 어디 갔나" 가 된다
  - ⚠️ 계류 일수는 SQL 이 준다 (`CURRENT_DATE - propose_dt`). JS 로 날짜를 빼지 말 것 — 프로젝트의 로컬 getter 금지 규칙
  - 🔴 **가결에 초록·부결에 빨강을 쓰지 않는다.** 이 쟁점에서 통과가 좋은 일인지 나쁜 일인지는 우리가 정할 게 아니다. 완료만 골드, 나머지 무채색
- 🔴 **위원장 대안 법안은 DB 에 없다** (2026-08-23 실측: 전체 18,830건 중 이름에 `(대안)` **0건**, `proposer_name ILIKE '%위원장%'` **0건**).
  원천(`nzmimeepazxkubdpn`)이 **의원발의만** 주고 위원장 대안을 안 준다. 그래서 `대안반영폐기` 건이 **어떤 대안으로 흡수됐는지 답할 수 없다.**
  화면이 그 한계를 문장으로 밝힌다 — 지우지 말 것. 링크를 걸고 싶으면 원천부터 찾아야 한다 (`ALLBILL` 계열 확인 필요)
- 해설(5-Zone) 커버리지는 쟁점당 3~5건이면 페이지가 "답이 있는" 느낌이 된다 (실측 전세사기 102건 중 5건).
  고를 때는 **유일한 통과 건 → 공동발의 최다 → 최근 대안반영폐기 → 최근 계류 → 다른 법률 대표** 순이 잘 먹혔다. 건당 $0.018
- 🔴 **「무엇이 달라지나」(`.iss-impact`)가 쟁점 페이지의 존재 이유다.** 사용자 지적(2026-08-23):
  *"쟁점으로 눈길을 끌어 눌러보게 했다면, 그 쟁점이 결국 **내 삶에 어떤 영향**을 끼치는지가 중요한 정보일 것 같다."*
  🔴 **그 재료는 이미 DB 에 있었다** — 5-Zone 의 `changes`(지금은/바뀌면)와 `affected`(혜택·손해·`direct`)가 정확히 그것인데
  법안 상세에만 갇혀 있었다. `affected.direct` 는 `["전세 임차인","임대인","부동산 중개사"]` 처럼 **실제 사람 목록**이다
  - **통과한 것 먼저** (`이미 바뀌었습니다` / `통과되면 이렇게 바뀝니다`) — 확정된 변화가 가정보다 강한 정보다.
    실측 전세사기: `이미 바뀌었습니다 1건`(주택도시기금법, 전세보증금 회수 신속화) + `통과되면 4건`
  - ⚠️ `changes` 값에는 `<strong>` 이 들어 있다 (상세는 `renderRichText` 로 렌더). 여기선 **태그를 걷어내고 자른다** —
    자르는 길이가 태그 중간에 걸리면 마크업이 깨진다. 강조를 살리려면 **서버용 `renderRichText` 부터** 만들 것
  - 🔴 **`좋아지는 쪽`·`부담지는 쪽`에 초록·빨강을 쓰지 않는다.** Zone 3 에서 "찬성/반대는 입장이라 색을 다르게 주는 순간
    어느 쪽을 지지한다는 신호가 된다" 고 정한 것과 같은 이유. 구분은 라벨과 표면 명도로만
  - 🔴 **카드는 앞 3장까지, 나머지는 헤드라인 한 줄이다** (2026-08-23 3차). 해설이 늘면 이 섹션도 늘어진다 —
    실측 카드 **309px**/장이라 20건이면 섹션만 **6,620px** (법안 목록보다 커진다).
    캡을 걸면 **2,172px** 로 줄고, **제목 20개는 전부 보인다**(한 줄 39px).
    - 🔴 **접지 말 것.** 이 섹션은 **읽히는 게 목적**이라 `details` 로 감추면 안 읽힌다 —
      법안 목록과 성격이 다르다 (거긴 "전부 공개" 가 목적이라 접는 게 맞았다).
      제목은 다 보이고 **자세히는 법안 상세(5-Zone)에서** 본다는 분담이다
    - 🔴 **「이미 바뀌었습니다」는 전부 카드로.** 확정된 변화라 수가 적고(실측 0~2건)
      이 쟁점에서 실제로 일어난 일이라 가장 중요하다. 캡은 「통과되면」에만 건다
  - 🔴 **해설 대상은 「그 쟁점의 중심 법률」에서 고른다 — 공동발의 수만 보면 안 된다** (2026-08-23 실측).
    공동발의 최다로 뽑았더니 법원행정처 쟁점에 `근로기준법`·`외국인아동 출생등록`,
    국민연금에 `북한이탈주민`·`상법` 이 뽑혔다. 키워드가 본문에 스쳐 지나간 법안들이다.
    → **계열(같은 `bill_name`) 5건 이상인 법률**에서만 고른다. 곁가지는 계열이 2~3건이라 자연히 빠진다
    - ⚠️ **비중(%)으로 자르면 안 된다** — 검찰 수사권은 법률이 93종으로 흩어져 최대가 13%다. 절대 건수로 볼 것
    - ⚠️ **법률당 상한을 걸지 말 것** — 지배적 법률이 곧 그 쟁점이다 (국민연금법 102건 · 소년법 8건).
      상한 4를 걸었더니 국민연금법이 막히고 `근로자퇴직급여`·`고령자고용` 이 대신 뽑혔다
    - ⚠️ **8건을 억지로 채우지 않는다.** 법원행정처는 중심 법률(법원조직법)이 7건뿐이라 **7건에서 멈춘다** —
      8번째를 노이즈에서 채우느니 7이 낫다
  - 🔴 **커버리지를 숫자로 밝힌다** (`해설이 있는 5건에서 뽑았습니다 (전체 102건 중)`). 안 밝히면 "이 쟁점의 영향은 이게 전부" 로 읽힌다.
    그리고 이 칸은 **AI 산출물이라 사실과 다를 수 있다**는 고지를 섹션 안에 따로 둔다 (사람 삶에 대한 서술이라 페이지 하단 각주로는 부족하다)
- 🔴 **무엇을 쟁점으로 삼나 — 기준은 `utils/issues.js` 주석에 박혀 있다** (2026-08-23 확정).
  자동 선정은 안 된다(신호 셋 중 둘 실패: 법률 집중도는 **그릇**이지 쟁점이 아니고, 브리핑 threads 는 카드 23건이라 주제가 전부 1일).
  본회의 반대표만 유효한데 20표 이상이 602건 중 **14건뿐**이고 사후적이다. → **자동화 대신 명시화**하고 고른 이유(`why`)를 화면에 노출한다
  - 필수: 법안 10건+ · 정당 2곳+ · 최근 12개월 발의 있음 · 계류 남아 있음 · 제목을 법률명·제도명으로 쓸 수 있음 · 키워드 3글자+
  - 가산: 본회의 반대표 20+ · 계류 평균 1년+
  - 🔴 **"뉴스에선 시끄러운데" 를 근거로 쓰지 말 것 — 우리는 화제성을 측정하지 않는다.**
    기사를 수집하지 않고(설계), 검색량도 없고, 쟁점 페이지 방문 통계도 아직 없다.
    실제로 한 번 새어들었다 — 킥 섹션의 값을 *"뉴스에선 시끄러운데 국회는 조용하다"* 로 설명했는데
    **앞의 절반이 근거 없는 단정**이었다 (사용자 지적 2026-08-23). 문서 3곳에서 걷어냈다.
    🔴 **대비는 읽는 사람의 몫이다** — 방문자는 이미 뉴스를 보고 왔으니 "시끄럽다" 는 그쪽이 안다.
    우리는 그 사람이 없는 절반(기록)만 대면 된다. 화면이 `위원회 0/12 · 전체 26.1%` 라고만 해도
    "어? 뉴스에선 맨날 나오는데" 는 독자가 스스로 붙인다
    ⚠️ 단 `뉴스가 아무리 시끄러워도 발의가 없으면 안 만든다` 는 괜찮다 — **배제 규칙**이지 화제성 주장이 아니다
  - 🔴 **여론의 프레임을 이름으로 쓰지 말 것.** 같은 법안 107건을 `친북 정책 논란` 이라 부르면 한쪽 프레임 채택이고,
    `남북 교류·협력` 이라 부르면 **양쪽 프레임으로 들어온 사람이 같은 기록을 본다.** 후자여야 이 사이트를 만든 이유가 산다
  - ⚠️ **관심은 있어 보이는데 법안이 거의 없는 주제가 있다** (실측: 외국인 건강보험 **0건** · 외국인 선거권 **1건**). 기준 ①에 걸려 쟁점으로 안 만든다
- 🔴 **인물·사건명 특검법은 어느 쟁점에서든 뺀다** (`EXCLUDE_PATTERN = '특별검사|국정조사|탄핵소추'`, DAO 가 `bill_name !~` 로 적용).
  「검찰 수사권 조정」을 만들었더니 **"본회의를 통과했습니다" 맨 위 네 건이 전부 특검법**이었다 — 본문에 `수사권` 이 당연히 들어가 걸리는데
  이름에 인물명이 박혀 있어 가장 눈에 띄는 자리에 인물명이 늘어섰다. 화면 각주가 뺐다고 밝힌다 (숨긴 게 아니라 단위가 다르다)
  - 🔴 **`진상규명` 을 패턴에 넣지 말 것** — 과거사 법안이 통째로 걸린다 (제주4·3 9건 · 여수ㆍ순천 7건 · 이태원참사 5건 · 6·25 납북피해 6건).
    좁힌 패턴은 이들을 전부 유지한다 (실측 확인)
  - ⚠️ 대신 상설특검법(`특별검사의 임명 등에 관한 법률` 6건)도 같이 빠진다. 제도 법안이라 손실이지만 인물명 53건이 섞이는 비용이 더 크다고 봤다
  - ⚠️ `bill_name` 만 본다 — `summary` 까지 보면 "특검법을 언급한 제도 법안" 까지 사라진다
  - 실측: 검찰 수사권 173 → **120건** · 법원행정처 30 → 27건 · 전세사기·국민연금·남북 교류 **영향 0**
- 🔴 **제안이유 원문의 인물명은 그대로 둔다.** 검찰 수사권 페이지에 `이재명 정부는 검찰개혁을 명분으로…` 가 보이는데
  그건 **우리 문장이 아니라 국회 공식 문서의 제안이유**다 (국민의힘 정점식 안, 공동발의 110명). 발의자 서술을 중화하지 않는 것이 원칙이고
  (ANALYSIS.md 원칙 10), 그 옆에 반대 방향 안이 나란히 놓이는 것이 이 페이지의 설계다. **프레임은 우리가 쓰는 게 아니라 발의자가 쓴다**
- 쟁점 6개 (2026-08-23): 법원행정처·전세사기·촉법소년·검찰 수사권·남북 교류·국민연금.
  실측 법안 12~165건 · 영향카드 3~5장 · 375px 가로 오버플로 **0** · 각 페이지 각주 5줄
  - ⚠️ **해설을 새로 돌린 뒤 화면에 안 뜨면 캐시다.** `IssueService` 가 10분 TTL 이라
    배치 직후에는 옛 결과가 나온다. 서버를 재시작하거나 10분 기다릴 것 (실제로 한 번 헷갈렸다)
  - ⚠️ AI 분석 JSON 파싱 실패율이 실측 **약 4%** 다 (9건 중 1건). 실패해도 다음 실행에 자동 재편입되므로
    같은 `--bill-id` 로 한 번 더 돌리면 된다
- 🔴 **브리핑 ↔ 쟁점은 `bill_ids` 가 아니라 `날짜`로 잇는다** (2026-08-23). 카드의 `bill_ids` 는 그날의 **대표 5건**뿐이라
  그걸로 매칭하면 겹침을 크게 놓친다 — 실측 **bill_ids 기준 9/32 카드 → 날짜 기준 19/32(59%)**
  - `/briefing/:id` → **「이날 움직인 쟁점」** (`IssueService.getIssuesForDate(date)`, 쟁점 6개 병렬 + 10분 캐시).
    뉴스 검색 링크 **위**에 둔다 — 사이트 안에서 더 볼 것을 먼저 주고 밖으로 나가는 링크는 그다음
  - `/issue/:slug` → **「최근 이 쟁점이 움직인 날」** (`getIssueBriefings.sql`, 최근 5일). 쟁점 페이지는 22대 전체를 모은 것이라
    **지금도 움직이는지**가 안 보인다. 이게 그 신호이자 매일 갱신되는 콘텐츠로 가는 길이다
  - ⚠️ 두 방향 다 **실패해도 빈 배열**을 돌려 페이지를 살린다. 걸리는 쟁점이 없으면 섹션 자체를 안 그린다
  - ⚠️ `getIssuesForDate.sql` 은 `$1` 이 날짜라 키워드 자리표시자가 **`$2` 부터** 시작한다 (`buildMatch(kw, cols, offset)`)
  - 실측 TTFB: `/briefing/:id` **50ms** · `/issue/:slug` **10ms** (연결 추가 전후 차이 없음)
- 🔴 **쟁점 후보 발굴은 `/admin/issue-candidates`** (2026-08-23). **후보를 정해주는 화면이 아니다** —
  자동 선정이 안 된다는 게 결론이므로 사람이 고를 때 쓰는 **재료와 검사기**를 준다:
  | | |
  |---|---|
  | ① 키워드 검사기 | `?kw=상속세,증여세` GET 폼. 기준 5개를 자동 판정 + 걸리는 법률 목록 + **기존 쟁점과 겹침** |
  | ② 반대표 신호 | 반대 20표+ 법안. 이미 쟁점에 잡힌 건 `이미 쟁점에 포함` 표시 (실측 11건 중 미커버 9) |
  | ③ 브리핑 반복 주제 | 2일 이상 등장한 `threads.theme`. **지금은 0건** |
  | ④ 현재 쟁점 커버리지 | 해설 3건 미만이면 `보충 권장` — 새 쟁점보다 이쪽이 먼저다 |
  - 🔴 **기준 ⑤(제목을 법률명·제도명으로 쓸 수 있나)는 자동 판정에서 뺐다.** 사람만 판단할 수 있어서
    `?` 로 표시하고 "직접 확인" 이라고 쓴다 — 통과했다고 표시하면 거짓이 된다
  - 🔴 **③이 비어도 감추지 않는다.** 왜 비었고(카드가 적어 주제가 대부분 1일) 언제 채워지는지를 문구로 쓴다
    (`/xray` 빈 지표 처리와 같은 판단 — 감추면 조용히 없는 기능이 된다)
  - ⚠️ `threads.theme` 은 AI 가 매일 새로 짓는 자유 문자열이라 표기가 조금만 달라도 다른 주제로 센다
    (`전세사기 지원` vs `전세사기 피해 지원`). 즉 ③의 숫자는 **과소 계상**이다. 정규화를 넣고 싶어지면
    그게 곧 주제 판별이고 **이미 실패한 문제**라는 걸 먼저 볼 것
  - 진입: `/admin/schedule` 하단 링크. 비로그인은 **404** (관리자 페이지 존재를 숨긴다)
- 🔴 **`/issue` 목록은 카드가 아니라 행(row)이다** (2026-08-23 재구성). 쟁점이 늘어날수록 카드형은 무너진다 —
  설명 2~3줄짜리 카드가 130px 이라 20개면 목록만 2,700px 이 된다. **커뮤니티를 카드형에서 보드형으로 바꾼 것과 같은 판단**
  (썸네일도 요약도 없는데 카드일 이유가 없다). 한 행에 `제목·부제 | 법안·계류·해설 | 최근 발의일`.
  실측 행 **83px 균일** · 6개 목록 541px · 20개로 늘어도 1,812px (모바일 1열 170px)
  - 🔴 **선정 원칙(`.iss-rule`)은 목록 위에 둔다.** 처음엔 맨 아래였는데, 그러면 스크롤한 사람만 본다 —
    "여기 있는 게 무슨 기준으로 골라졌나" 를 먼저 알아야 목록이 다르게 읽힌다 (사용자 지적)
  - 🔴 **기본 정렬은 `최근 움직인 순`**(`stats.lastDate`). 쟁점이 늘어나면 목록에서 가장 먼저 묻는 게
    "지금 뭐가 움직이나" 이지 "뭐가 크나" 가 아니다. `?sort=recent|bills|pending`, **모르는 값은 기본값으로 조용히 폴백**
    (`/xray/chart` 와 같은 판단). GET 링크라 JS 없이 동작한다
  - ⚠️ `lastDate`·`daysAgo` 는 **추가 쿼리 0** — `list()` 가 이미 캐시된 `get()` 결과를 재사용한다.
    오늘 날짜는 `Intl`(KST)로 뽑는다 (로컬 getter 금지)
  - ⚠️ 해설 3건 미만이면 숫자를 흐리게(`is-thin`) — 관리자 화면의 `보충 권장` 과 같은 신호
- 🔴 **상세 페이지 지면 배분 재구성** (2026-08-23 2차). 실측이 뒤집혀 있었다 (국민연금 기준):
  문서 **27,247px 중 법률별 법안 목록이 20,364px = 75%**. 리스트 33개 · 항목 194개.
  그런데 방문자 관심사에서 "법안 전체 목록" 은 **최하위**고 그건 `/bill` 이 이미 하는 일이다.
  | | 전 | 후 |
  |---|---|---|
  | 문서 높이 | 27,247px | **4,994px** |
  | 「무엇이 달라지나」 | 11% | **30%** |
  | 법률별 목록 | **75%** | 18% (요약 표 + 접힘) |
  | 보이는 리스트 · 항목 | 33 · 194 | **15 · 30** |
  - 🔴 **법안 목록은 `<details>` 로 접는다 — 숨기는 게 아니다.** 대신 **법률별 요약 표**(법률·건수·계류·기간)를
    항상 보여준다. "어떤 법률에 몰렸나" 는 한눈에 보이고, 개별 법안은 원하는 사람만 연다.
    `details` 라 **JS 없이 동작하고 검색엔진도 본문으로 읽는다**
  - 🔴 **순서는 방문자가 묻는 순서다**: 무슨 얘기야(요약) → 어떻게 됐어(결과) → **나한테 뭐가 달라져**(영향)
    → 누가 밀고 있어(의원) → 어떤 법률이야 → 지금도 움직여(브리핑) → 한계.
    **「어떻게 됐나」와 「무엇이 달라지나」 사이에 다른 섹션을 끼우지 말 것** — 이어서 읽혀야 한다
    (구 버전은 정당 분포가 사이에 끼어 흐름을 끊었다)
  - 구 4블록(`선정 근거`·`숫자 4칸`·`단위 각주`·`정당 분포`)은 **요약 카드 하나(`.iss-sum`)로 합쳤다** —
    블록이 쪼개져 있으면 스크롤만 늘고 읽히는 건 같다. 뉴스 검색 링크도 각주 안 한 줄로 흡수
- 🔴 **「이 쟁점에 이름을 많이 올린 의원」(`.iss-people`)** — 쟁점 페이지에서 **의원으로 가는 유일한 길**이다.
  구 버전은 "누가 밀고 있나" 에 답할 게 정당 분포뿐이었는데, 이 사이트 축은 "당 말고 사람" 이다.
  - 🔴 **순위표가 아니다.** 번호를 붙이지 않고 `1위` 같은 말을 쓰지 않는다. 공동발의는 **이름을 올린 것**이지
    기여도가 아니다 — 각주로 밝힌다 (브리핑이 "이름을 얼마나 걸었는지일 뿐" 이라 쓰는 것과 같은 규칙)
  - 🔴 **중앙값·총원을 같이 낸다** (`getIssueTopSigners.sql` 이 CTE 로 한 쿼리에). 숫자만 주면 39건이 많은 건지 알 수 없다
    (평균선 없는 막대와 같은 문제). 실측 국민연금 서명자 282명 · 중앙값 5건 · 최대 39건
  - ⚠️ 퇴임 의원은 `politicians` 조인이 비어 이름이 없다 → **링크를 걸지 않는다** (그 mona_cd 로 가면 상세가 404)
- 🔴 **접힌 「법안 전체 보기」 안은 평면 목록 + 페이징이다** (2026-08-23 3차). 안에서 또 법률별로 묶지 않는다 —
  "어떤 법률에 몰렸나" 는 **위 요약 표**가 이미 답하고, 165행을 통째로 SSR 하면 페이로드가 그대로다.
  **첫 20행만 SSR + 나머지는 `GET /api/issue/:slug/bills?page=N`**. 실측 SSR **162KB → 65KB** · API 한 페이지 7KB
  - 🔴 **추가 쿼리가 0이다** — `IssueService.get(slug)` 이 이미 전건을 캐시로 들고 있어 그걸 자른다.
    페이징이 DB 부하를 늘리지 않고 **페이로드만** 줄인다
  - ⚠️ **정렬을 다시 하지 말 것.** `getIssueBills.sql` 이 정한 순서 그대로 자른다 —
    다시 정렬하면 SSR 첫 페이지와 API 2페이지 경계가 어긋난다 (의원 상세에서 3건 중복이 났던 함정)
  - ⚠️ 뷰의 행 마크업과 인라인 스크립트의 `row()` 는 **같아야 한다** (`_activity_row.ejs` 와 같은 규칙).
    다르면 1페이지와 2페이지가 달라 보인다. 범위 밖 page 는 마지막으로 접고, 모르는 slug 는 404
- 🔴 **페이저는 `PB.renderPager` 하나다** (2026-08-23 `interactions.js` 로 공용화, CSS 는 `main.css` 의 `.pagination-inline`).
  의원 상세가 지역 구현을 갖고 있었는데 쟁점 페이지가 **세 번째 복사본**이 될 뻔했다. 페이지마다 복사하지 말 것
  - 🔴 **PB 가 없을 때를 반드시 처리할 것.** `interactions.js` 는 layout 본문 출력 **뒤에** 로드되는데
    의원 상세 **표결 내역 탭은 첫 페이지가 SSR 이라 파싱 중에** 페이저를 그린다 → 그 시점엔 PB 가 없다.
    공용화하면서 이걸 빠뜨려 `ReferenceError: PB is not defined` 로 **표결 탭 페이저가 통째로 안 그려졌다.**
    법안 활동 탭은 fetch 콜백 안이라 멀쩡해서 **한쪽만 깨진 걸 눈치채기 어려웠다.**
    → 호출부는 `window.PB` 를 확인하고 없으면 `DOMContentLoaded` 에서 다시 그린다
  - 🔴 **EJS 주석·문자열에 `<%` … `%>` 를 쓰지 말 것.** 위 주석을 쓰면서 layout 의 본문 출력 태그를
    그대로 적었다가 **페이지가 500** 이 났다. EJS 는 주석 안이든 문자열 안이든 텍스트로 찾아 스크립틀릿을 닫는다
    (발언 기록 섹션에서 이미 한 번 겪은 함정)
- 🔴 **「이 쟁점, 국회에서 굴러가고 있나」(`.iss-bench`)가 이 페이지의 킥이다** (2026-08-23 4차).
  사용자 지적: *"결국 AI 분석 법안으로 가는 링크네. 킥 정보가 부족한 거 같다."* 맞는 말이었다 —
  요약 숫자가 전부 **총량**(165건·282명·17법률)이라 **"많은 건가 적은 건가" 에 답을 못 했다.**
  총량은 놀랍지 않다. **전체 기준선과 나란히 놓는 순간 사실이 된다:**
  | 쟁점 | 위원회 문턱 넘음 | 두 거대정당 공동서명 | 계류 평균 |
  |---|---|---|---|
  | **전체 기준선** | **26.1%** | **4.8%** | **411일** |
  | 전세사기 | 39.2% ▲ | 4.9% | 463일 (+52) |
  | 국민연금 | 34.5% ▲ | 1.8% | 381일 (−30) |
  | 검찰 수사권 | 23.3% | 1.7% | 415일 |
  | 법원행정처 | 11.1% ▼ | **0%** | 401일 |
  | 남북 교류 | 5.0% ▼ | **11.7%** (평균의 2.4배) | 523일 (+112) |
  | **촉법소년** | **0/12** | **0%** | **532일 (+121)** |
  촉법소년은 **발의 12건 · 여야 공동 0 · 위원회 통과 0 · 532일 방치**다.
  전세사기(39.2% · 실제 1건 통과)와 정반대. **그 대비가 정보다.**
  - 🔴 **좋다/나쁘다를 쓰지 말 것.** 위원회 처리율은 발의가 많을수록 분모가 커져 낮아지고,
    두 정당이 같이 이름을 올렸다고 합의가 된 것도 아니다(반대 방향 안에 각자 서명했을 수 있다).
    각주가 이걸 밝히고, 색도 **골드·무채색**만 쓴다 (초록·빨강을 쓰는 순간 평가가 된다)
  - 🔴 **`여야` 대신 `두 거대 정당`** — 의석 대부분이지만 국회가 두 당뿐인 건 아니다
  - ⚠️ **"2개 정당 이상" 을 초당성 지표로 쓰지 말 것** — 무소속만 껴도 성립해 전 쟁점이 38~70% 로 나온다.
    민주 **AND** 국힘 동시 서명이라야 갈린다 (전체 4.8%)
  - ⚠️ 전체 기준선(`getGlobalBenchmark.sql`)은 **전건 18,830행 스캔**이라 쟁점과 무관하게 **따로 캐시**한다.
    쟁점마다 돌리면 안 된다. 실측 TTFB 6~7ms (캐시 적중)
  - ⚠️ 인물·사건명 특검법을 **양쪽에서 같이** 뺀다 — 모수가 같아야 비교가 성립한다
  - ⚠️ 파생 섹션(「무엇이 달라지나」)은 29% 로 남는다. 리스트를 재배치해도 그 인상은 안 바뀌므로
    **비중이 아니라 킥으로 푼 것**이다. 픽셀을 더 깎으려 하기 전에 이 판단을 먼저 볼 것
- 🔴 **상세에 앵커 내비 `.iss-toc` 가 있다** (2026-08-24). 섹션이 8개인데 훑을 수단이 없었다 —
  법안 상세엔 sticky 인덱스, 의원 상세엔 `.ov-toc` 가 있는데 **쟁점만 빠져 있었다.**
  **활성 추적 방식**은 `.ov-toc` 와 같게 맞췄다 (고칠 일이 생기면 두 곳을 같이 볼 것).
  **생김새는 다르다** — 아래 「밑줄 탭」 참조
  - 🔴 **칩은 JS 가 `data-toc` 를 읽어 만든다. 목록을 손으로 적지 말 것** — 섹션 절반이
    조건부(bench·impact·people·brief)라 SSR 로 적어두면 없는 곳으로 보내는 칩이 생긴다
    (실측: 전세사기 8개 · 법원행정처 **7개** — 브리핑이 없어 자동으로 빠진다)
  - 🔴 **`top` 은 `var(--nav-h)`** (데스크톱 100 · 모바일 60). 60px 하드코딩 금지
  - 🔴 **배경·보더는 바깥이 아니라 가운데 정렬된 `-inner` 에 준다.** 바깥에 주면
    **흰 띠만 화면 끝까지 뻗어** 본문과 따로 논다 (1920px 실측: 띠 1910 vs 본문 1000 —
    양쪽 455px 씩 넘쳤다. 사용자 지적으로 잡았다). 사이트의 다른 sticky 바도 같다 —
    `/bill`·`/politician` 의 `.filter-bar`·`.status-tabs` 는 바깥이 **투명 full-bleed** 이고
    흰 배경·보더는 `-inner` 가 갖는다. 의원 상세 `.ov-toc`, 법안 상세 `.bd-toc` 도 마찬가지
    - 🔴 **이 규칙은 사이트 전체에 적용된다.** 2026-08-24 에 법안 상세를 가로 탭 바로 바꾸면서
      "마진노트를 덮어야 한다" 는 이유로 바깥에 흰 배경을 줬다가 **같은 지적을 또 받았다.**
      덮개가 필요하면 흰색이 아니라 **`--bg`(페이지 배경)** 로 채울 것 — 눈에는 배경 그대로라
      띠처럼 보이지 않으면서 가려주기만 한다
  - ⚠️ 바깥이 투명해도 스크롤 시 본문이 비치지 않는 이유 — 콘텐츠가 `.iss-wrap` 1000px 안에만
    있어서 같은 1000px 인 inner 흰 띠가 전부 덮는다. **폭을 바꾸면 이 전제부터 다시 볼 것**
  - 🔴 **제목·칩·본문이 한 기준선을 갖게 셋을 같이 맞춘다** (2026-08-24). 공용
    `.pb-page-header-inner` 는 **1280px** 인데 이 페이지 본문은 **1000px** 이라
    제목이 본문보다 164px 왼쪽에 있었다 (기존 상태). issue.css 에서 스코프 오버라이드 —
    `.pb-page-header { padding-left/right: 0 }` + `-inner { max-width: 1000px; padding: 0 24px }`
    로 `.iss-wrap` 과 **글자 그대로 같은 상자**로 만든다
    - ⚠️ 부모 패딩을 0 으로 안 내리면 **좁은 화면에서 24+24=48px 로 더해져** 제목만 안쪽으로 밀린다
      (1024px 실측: 제목 48 vs 본문 31). 넓은 화면은 max-width 로 가운데 정렬돼 안 드러난다
    - ⚠️ 모바일(≤768) 은 `.iss-wrap` 이 16px 이라 `-inner` 도 **16px 로 같이** 내린다.
      하나만 빠뜨리면 그것만 어긋난다 (768px 실측: 제목 24 vs 본문 16)
    - ⚠️ 세로 패딩·`border-top` 투명 트릭·그라디언트는 건드리지 않는다 (full-bleed 부모여야 성립)
    - 🔴 issue.css 는 쟁점 페이지에서만 로드된다 — **공용 `.pb-page-header-inner` 를 고치지 말 것**
      (`/bill`·`/politician`·`/community`·`/about` 은 1280px 그대로여야 한다. 실측 확인)
  - 🔴 **`pinned` 을 지우지 말 것** — 마지막 두어 섹션이 짧으면 **둘 다 마지막 화면에 들어와**
    어떤 스크롤 위치로도 갈리지 않는다. 스크롤만 보는 순수 함수로는 원리적으로 구분이 안 되므로
    **누른 칩을 기억해** 바닥 구간에서만 존중한다. 없으면 「최근 움직임」 을 눌러도 「한계」 가 켜진다
    (실측 1280×720 · 전세사기. 800×454 에선 우연히 안 나서 놓치기 쉽다)
  - ⚠️ 첫 표시에 `requestAnimationFrame` 을 쓰지 말 것 — 배경 탭에서 콜백이 안 돌아
    **활성 칩이 하나도 없는 채로** 뜬다 (의원 상세에서 실제로 겪은 것)
  - 🔴 **생김새는 밑줄 탭이다. 알약 칩으로 되돌리지 말 것** (2026-08-24 사용자 지적: "sticky 가 혼자 다르다").
    사이트의 **주 sticky 메뉴는 전부 밑줄 탭**이다 — `/bill` `.status-tab` · `/politician` `.party-tab` ·
    `/politician/:id` `.tab-btn`. 셋 다 `padding: 14~16px 22px` · `14px/500` · 활성은 `border-bottom: 2px` 골드이고,
    컨테이너는 `padding: 0 24px` · 흰 배경 · `border-bottom: 1px` · **gap 없음**이다.
    - ⚠️ `.ov-toc-link` 의 알약 칩을 그대로 가져왔던 게 원인이었다. 거긴 **탭바 아래 둘째 줄**이라
      위와 구분하려고 칩인 것이고, 쟁점 상세는 탭바가 없어 **이 줄이 곧 주 메뉴**다
    - ⚠️ 활성색은 `--accent`(흰 배경 3.79 미달)가 아니라 **`--text`** 다. 골드는 밑줄이 맡는다
    - ⚠️ 높이 **50/51px 은 `/politician` `.party-tab` 과 정확히 같다** (둘 다 `<button>`).
      `/bill` 이 55/56 인 건 그쪽이 `<a>` 라 줄높이를 상속받아서다 — **값을 복사하지 말 것**(기존 기록)
  - 실측 2026-08-24: 탭 8개 825px(데스크톱 1000px 안에 한 줄 · 모바일 858px 가로 스크롤) ·
    바 높이 **51px** · 클릭 착지 후 섹션 상단이 고정 크롬 아래 12px ·
    **320·375·768·1024·1280·1440·1920px 전부** 제목=탭=본문 좌측 기준선 일치 · 문서 가로 오버플로 **0**
- 쟁점 추가 = `utils/issues.js` 에 한 항목. 목록·상세·사이트맵이 자동으로 따라온다
...
- **진단 캐러셀 `/promo/balance`** (2026-08-26, 백로그 2) — 성향 진단 홍보 6장: `훅(N명 중 가장 가까운 사람은?) → 보수/진보 하나로 안 됨 → 세 축(AXIS_META) →
  익명 점구름 지도(getAxisCloud, 사분면 = 유형 4종 이름, 중앙 골드 ?) → 결과 목업 → 로그인 없이 5분`.
  🔴 **TOP3 장은 실명이 아니라 마스킹 목업** — 계정이 의원을 골라 보이는 순간 편집이 된다 (SNS.md 하지 말 것 2). 목업임을 카드 안에 문장으로 밝힌다
  🔴 의원 수(269)·문항 수(20)는 **렌더 시 DB** (axis cloud 길이 · `balance_game_packs.question_count`) — 문항 교체·매핑 갱신으로 움직이는 값이다
- **쓰레드 문안 뱅크 `/promo/threads`** (2026-08-26) — 대화형 포스트 10개(상식 뒤집기 5 · 숫자 질문 3 · 당신이라면 2) + 셀프 답글 + 복사 버튼 + 500자 카운트.
  공식 `질문 → 반전 → 데이터 → (셀프 답글에서) 당말사` — 🔴 **첫 포스트에 링크를 넣지 않는다.** 숫자는 렌더 시 DB 라 **게시 직전에 새로고침**해서 복사한다.
  - 🔴 **링크는 홈이 아니라 그 주장을 보여주는 페이지로 보낸다** (2026-08-27). 홈으로 보내면 방금 읽은 숫자를
    방문자가 스스로 찾아야 하는데 홈 B 섹션은 히어로를 지나 스크롤해야 나온다 —
    "실제 숫자는 당말사에 있습니다" 라고 해놓고 못 찾으면 그 글은 광고가 된다.
    `/xray#xr-<id>` 는 그 섹션만 펼쳐서 열리고 `/politician?sort=gap-desc` 는 정렬된 채로 열린다.
    실측 착지점: 문안 1 `#xr-funnel` · 2 `/guide/why-so-few-no-votes` · 3 `/politician?sort=gap-desc` ·
    4 `#xr-gapdist` · 9 `/#mine` · 10 `/guide/how-a-bill-moves`. **홈으로 되돌리지 말 것**
  - 🔴 **셀프 답글은 본문 직후 내가 이어 다는 2단이다. 남의 댓글을 기다리지 말 것** (2026-08-26 수정). 처음엔 기다리는 설계였는데
    계정 초기엔 댓글이 안 달려 **링크가 영영 안 나간다** — 유일한 전환 경로를 남의 행동에 걸어둔 꼴이었다.
    문안 10개의 reply 가 전부 `"그럼 …인가?" 이것도 함정입니다` 처럼 **가상의 반문에 스스로 답하는 셀프 답글 문체**라 라벨과도 어긋나 있었다.
    링크가 첫 포스트를 피하는 것으로 목적(광고로 안 읽히기·도달 페널티 회피)은 이미 달성된다
  - ⚠️ **브리핑 short 체인은 정반대로 링크를 1번에 둔다** (`utils/threadsPost.js` — 펼치지 않아도 유입되게). 둘은 성격이 다르다:
    브리핑은 **배포물**(500자를 넘어 쪼갠 것, 한 번에 다 올림) · 문안 뱅크는 **대화 미끼**(1개로 완결, 링크는 답글로). 한쪽 규칙을 다른 쪽에 옮기지 말 것
  - ⚠️ **답글이 두 종류다** — 뱅크의 **셀프 답글**(링크 배달, 본문 직후) / 일일 루틴의 **대화 답글**(실제 댓글에 기준 제시 화법, 밤에 몰아서). 뭉뚱그리지 말 것
  - **하단 「대화 답글 화법」 7종** (2026-08-26 추가, `REPLIES`) — 동의·공감 / 숫자 의심 / 특정 정당·의원 비난 / 중립성 의심 / 기능 질문 / 사실 오류 지적 / 싸움 걸기.
    유형마다 `상대가 한 말 · 뼈대 · 예시 · 하지 말 것`. 숫자는 셀프 답글과 같은 렌더 시 DB 값을 쓴다
    - 🔴 **문안이 아니라 뼈대다** — 상대가 쓴 말에 맞춰 고쳐 쓴다. 복붙 답글은 티가 난다
    - 🔴 **`seen` 이 유일한 신뢰 표시다.** 현재 `겪음` 1종 · `예상` 6종 — 나머지는 겪은 게 아니라 **상상해서 쓴 것**이다.
      실물 댓글이 오면 **그 문장으로 갈아끼울 것** (SNS.md 백로그 5). 상상한 문안이 헛돈 선례가 브리핑 v1 프롬프트다
    - 🔴 **상대의 아이디·프로필로 성향·지역을 추측하지 말 것** — "당 말고 사람" 의 정면 위반이고, 무엇보다 상대가 기분 나쁘다. 댓글에 쓰인 말에만 답한다
    - 🔴 **좋아요는 동의가 아니라 "읽었다" 는 신호다** — 답글을 다는 모든 댓글에 누르고, 진지한 댓글이면 **반대 의견이어도** 누른다.
      욕설·인신공격·특정인 비방·명백한 허위에는 안 누른다. **우리 결론에 동의한 댓글만 골라 누르면 그 순간 좋아요가 동의 표시로 바뀐다** (SNS.md 5-1장)
  - 🔴 **게시 시각: 브리핑은 오전, 주간 슬롯은 점심 12~13시** (인스타·쓰레드 같은 리듬). 체인 3개가 끝난 뒤 · 최소 4시간 —
    슬롯 있는 날은 계정 글이 5개(체인 3 + 본문 1 + 셀프 답글 1)라 붙여 올리면 우리 글끼리 도달을 먹는다.
    점심인 이유는 **답글 대응 사이클이 하루 안에 닫혀서**다 (저녁 게시는 답글이 밤~새벽에 쌓여 대응이 다음 날로 밀린다).
    ⚠️ 실측이 아니라 구조적 출발값이다 — SNS.md 성과표의 `시각` 칸이 4주 뒤 검증한다. 그전까지 바꾸지 말 것
  - ⚠️ 주간 슬롯은 **월(숫자) · 화(쟁점) · 수·목 중 하루(진단)** 세 번이다 (수·목 둘 다가 아니다)
  - **하단 「쟁점 문안」 6종** (2026-08-26 추가, `ISSUE_HOOKS` + `buildIssuePost`) — `/issue` 6개를 `IssueService.get`(10분 캐시)에서 받아
    `훅 → 방향이 여럿이라는 사실 → 위원회 처리율 vs 전체 → 계류·최장 대기` 로 조립한다. 셀프 답글은 그 쟁점 페이지 링크
    - 🔴 **뉴스에 대응하는 글이 아니다.** 판별 기준은 **뉴스를 몰라도 읽히는 글인가** — `논란`·`요즘`·`지금 시끄러운` 을 쓰지 않는다.
      화제가 된 게시물을 **인용·반박·팩트체크하지 않는다** (인용하면 그쪽 프레임을 우리가 유통시키고, 판정하면 편이 생긴다).
      밖이 시끄러워도 **정규 슬롯 차례에** 올린다 — 몇 시간 안에 올리면 그게 반응이다
    - 🔴 **정당별 건수를 문안에 쓰지 않는다** — 의석수가 달라 그대로 비교하면 오해인데 500자에 그 각주가 안 들어간다.
      `N개 정당이 각자 안을 냈다` 까지만 (쟁점 페이지의 `why` 와 같은 형태)
    - ⚠️ **법안 건수는 키워드로 모은 값이라 곁가지가 섞여 있다** (실측: 법원행정처 27건에 `예금자보호법`·`민법`·`전자정부법`,
      그리고 "본회의 통과 1건" 이 그 예금자보호법이다). **셀프 답글의 쟁점 페이지 링크가 필수** — 따지면 목록을 그대로 보여줄 수 있어야 한다.
      노이즈가 분자·분모에 같이 들어가므로 위원회 처리율은 **보수적인 값**이다
    - ⚠️ **금요일에 올리지 말 것** — 답글이 제일 많이 붙을 주제라 주말에 쌓이면 월요일까지 방치된다 (점심 게시와 같은 이유)
    - ⚠️ 한 쟁점 조회가 실패해도 나머지는 낸다 (`Promise.all` + catch → 그 쟁점만 빠진다)
  ⚠️ 문안 4(거수기)는 그 말을 **안 쓰기로 했다는 것** 자체가 내용 — 평가어 문안으로 고치지 말 것. 문안 추가 = `getThreadsBank` 의 `POSTS` 에 항목 하나
- **브랜드 소개 캐러셀 `/promo/intro`** (2026-08-26, 30일 플랜 1주차) — 6장: `훅(내가 뽑은 사람 지금 뭘 할까) → 정체(브랜드 락업 + /about 첫 문장) →
  무엇이 있나(의원 N명·법안 N건, 렌더 시 DB) → 하지 않는 것(정당색·평가·기사 인용 ✕) → 모르는 것도 적어둡니다(!) → 마무리`.
  🔴 **「모르는 것도 적어둡니다」 장을 빼지 말 것** — 차별점은 데이터가 많다가 아니라 무엇을 모르는지 밝힌다는 것 (/about 재작성 결론).
  문안은 /about 과 같은 말 — 여기서 새 주장을 만들지 않는다
...
##### PNG 저장 버튼 — `public/scripts/cardShot.js` (2026-08-22)
`/briefing/:id/card` **미리보기**에 카드마다 `PNG 저장` + 툴바 `PNG 전부 저장 (N장)`.
배치(`npm run insta`)나 DevTools 노드 캡처 없이 **브라우저에서 그대로 받는다** (폰 포함).

- 🔴 **캡처 방식 — SVG `<foreignObject>` 에 카드를 통째로 넣어 브라우저가 그린다** → canvas → PNG.
  의존성 0 (html2canvas·puppeteer 없음), 서버에 크롬을 올릴 필요가 없고,
  무엇보다 **미리보기와 산출물이 같은 DOM·같은 CSS** 라 "미리보기는 멀쩡한데 저장한 게 깨지는" 일이 없다
- 🔴 **SVG 이미지 안에서는 외부 리소스를 못 받는다** (폰트·이미지 fetch 가 아예 안 일어난다). 그리기 전에 둘을 심는다:
  ① **웹폰트** — Google Fonts `css2?...&text=` 로 **카드에 실제로 쓰인 글자만** 받아 base64 인라인.
     한글 전체는 수 MB 지만 이 방식은 **파일 3개 · 수 KB** 다 (Noto 는 가변폰트라 굵기별로 안 나뉜다).
     🔴 안 심으면 에러 없이 시스템 폰트로 폴백해 **눈으로만 이상한** PNG 가 나간다
  ② **브랜드 SVG 3종** — 같은 출처라 fetch → data URI. 외부 참조가 하나라도 남으면 그 자리가 빈 채로 찍힌다
  ⚠️ 폰트 목록은 뷰의 `<link>` href 를 재료로 만든다 — 카드 폰트를 바꾸면 자동으로 따라간다 (목록을 두 곳에 적지 않는다)
- 🔴 **`?slide=N`·`?story=1` 에는 이 스크립트를 싣지 않는다.** 그쪽은 문서 자체가 캔버스라
  버튼 하나만 끼어도 배치 PNG 가 오염된다. 미리보기 전용 요소에는 **`data-shot-omit`** 을 달 것 —
  클론에서 제거한다 (`body.story .sl-sticker-hint{display:none}` 같은 **조상 선택자는 클론 안에서 안 먹는다**)
- 🔴 **빈 PNG 를 조용히 저장하지 않는다.** foreignObject 지원이 부실한 브라우저는 에러 없이 하얀 PNG 를 준다 →
  1/10 축소본의 잉크 비율이 **1% 미만이면 실패로 알린다** (실측 정상 3.6~8.4% · 빈 카드 0%)
- 파일명은 배치와 같은 규칙 (`2026-08-20-01.png` … `-story.png`). 폰(`hover:none and pointer:coarse`)에서는
  다운로드 대신 **공유 시트**로 — 데스크톱까지 시트를 띄우면 7장 받을 때마다 시트가 뜬다 (맥 사파리·크롬도 `navigator.share` 가 있다)
- 🔴 **`전부 저장` 은 데스크톱과 모바일이 갈린다** (2026-08-22 실기기 보고: "모바일에선 첫 번째만 받아진다").
  모바일 브라우저는 프로그래매틱 다운로드를 **사용자 제스처당 한 개만** 허용해서 `a.download` 를 연달아 부르면
  첫 장만 저장되고 나머지는 **조용히 버려진다.**
  | | |
  |---|---|
  | 데스크톱 | 한 장씩 순차 다운로드. ⚠️ 브라우저가 **"여러 파일 다운로드"** 를 한 번 물어본다 (허용해야 2장째부터) |
  | 모바일 | 다 만들어 두고 **공유 시트 한 번**으로 전부 넘긴다 (사진 앱에 7장이 한꺼번에) |
  - 🔴 **모바일은 두 번 탭이다** (`만들기` → 라벨이 `N장 공유하기` 로 바뀜 → `공유`). 만드는 데 1~2초가 걸리는데
    `navigator.share` 는 **제스처 권한(transient activation, 약 5초)** 안에서만 열려서, 같은 탭에서 await 뒤에 부르면
    실패한다. 파일을 앞선 탭에서 만들어 두면 두 번째 탭은 await 없이 바로 시트를 연다 (shareCard.js 와 같은 수법)
  - ⚠️ **취소(AbortError)는 에러가 아니다** — 만들어 둔 파일을 버리지 말 것. 다시 누르면 재생성 없이 그대로 보낸다
  - ⚠️ 공유 시트 자체가 없으면(`navigator.canShare` 부재) **만들기 전에** 접는다. 7장을 만든 뒤 못 준다고 말하면 2초를 버린다
  - ⚠️ 카드 한 장짜리 `PNG 저장` 은 그대로 한 번 탭이다 (약 0.7초라 권한 안에 든다).
    그래도 share 가 실패하면 **저장으로 떨어진다** (`save()` 의 catch 가 download 로 흘려보낸다)
- 🔴 **쓰레드 이미지 모드(`/briefing/:id/threads?mode=image`)에도 같은 버튼이 있다** — 첨부할 표지 한 장을
  **미리보기까지 띄우고 그 자리에서** 받는다. 카드 페이지로 보내면 폰에서 탭을 오가다 첨부를 빠뜨린다
  (이 모드는 이미지를 안 붙이면 1번 게시물에 헤드라인이 없어 무슨 글인지 알 수 없다)
  - 🔴 **카드 마크업·CSS 를 그 페이지에 복사하지 않는다.** 같은 출처 **iframe** 으로 `?slide=1` 을 그대로 물어와
    `PBCard.frameCard(iframe)` 이 그 안의 `.sl` 을 넘긴다 (`cardShot.js` 는 `node.ownerDocument` 에서
    스타일·폰트 link 를 모으므로 iframe 문서든 최상위든 같게 동작한다). 복사하면 카드가 바뀔 때 두 곳이 갈린다
  - ⚠️ iframe 은 **1080×1350 그대로 두고 감싼 상자를 `transform: scale()` 로 줄인다.** iframe 자체를 작게 잡으면
    카드가 그 폭에 맞춰 다시 흐르는 게 아니라 스크롤이 생긴다. ≤560px 은 위아래로 쌓는다 (옆에 두면 글이 90px 폭)
  - 파일명은 `…-01.png` 로 카드 페이지·배치와 같다
- ✅ 실측 2026-08-22: 7장 규격 정확(1080×1350 · 1080×1920), 첫 장 688ms(폰트 fetch 포함) 이후 56~85ms.
  같은 날 배치 산출물과 픽셀 대조 결과 **글자 위치 완전 일치**, 차이는 글리프 가장자리(안티에일리어싱)뿐.
  쓰레드 iframe 경로도 **카드 페이지와 완전히 같은 결과**(대조 7.5% 동일 · 잉크 8.4% 동일)
- ⚠️ 배치를 없애지 말 것 — 영상(`genBriefingVideo`)이 PNG 파일을 재료로 쓰고, 자동화(`/api/briefing/export`)도 URL 을 넘긴다
...
##### 퀴즈형 쇼츠 — `batch/genQuizVideo.js` (2026-08-17)
브리핑 쇼츠는 처음부터 끝까지 **서술**이라 재미가 없다 (사용자: "재미가 없다"). 재미의 최소 단위는 `질문 → 공백 → 답` 인데
브리핑엔 시청자가 궁금해할 틈이 없고, 시청자("너")가 등장하지 않으며, 평가어 금지 원칙 때문에 숫자에 감정이 안 생긴다.
→ **예상을 먼저 세우게 하고 데이터로 깬다.** 우리는 평가하지 않고 예상만 깨므로 중립 원칙과 충돌하지 않는다.
- 첫 편 `own-vs-other`: `Q1 자기 당 법안 찬성? (보기 3) → 3·2·1 → 99.7% (48%가 100%) → Q2 상대 당은? → 3·2·1 → 96.1%, 격차 3.4%p →
  왜? 본회의 반대 0.66% (걸러지는 곳은 위원회) → 격차 30.9%p 의원도 있다, 누굴까 → 당말사`. 실측 **49초**
- 🔴 **숫자는 렌더 시 DB 에서 읽는다** (`politician_cross_party_vote` 코호트 중앙값 · `bill_votes` 반대 비율). 보기의 정답 항목도 데이터에서 만든다
  (`99% 이상`·`약 96%`) — 화면에 하드코딩하면 배치가 움직일 때 조용히 거짓이 된다
- 🔴 **사람 이름을 안 부른다** — "격차 30.9%p 인 의원도 있다, 누굴까" 까지만. 순위는 사이트(`/xray#xr-gapdist`)에 있다. 설명란에 다수당·소수당 의사일정 각주 필수
- 카운트다운은 1초 × 3 장면(째깍 880Hz), 말 없음 · 자막 없음. 보기는 컷마다 한 줄씩, 정답 장면은 나머지 보기 흐림
- **`--ep person` 사람 퀴즈** (2026-08-18) — 「우리 지역구 의원, 얼마나 일할까?」 같은 틀에 사람만 바꾼다 (× 309, 소재 고갈 대응).
...
| 폭 | 문서 | 분석 탭 |
|---|---|---|
| 1270px | 4,253px (5.3화면) | 3,564px |
| **375px** | **8,066px** | **6,915px** |

2단(`.vp-grid`·`.pr-grid`·`.pc-grid`)이 1단으로 접히면서 두 배가 된다. 카드 넷은 서로
**독립된 질문**(어떻게 투표했나 / 뭘 발의했나 / 누구와 / 회의에서)이라 한 번에 다 읽는 사람이 없고,
목차(`.ov-toc`)가 이미 점프를 맡고 있다. → **≤768px 에서 첫 카드만 펼치고 나머지는 접는다.**
실측 **8,066 → 3,861px (52% 감소)** · 분석 탭 6,915 → 2,709px

- 🔴 **데스크톱은 건드리지 않는다** — 5.3화면이면 문제가 아니고 2단 레이아웃이 접기와 안 어울린다
- 🔴 **기본은 펼침(HTML)이고 JS 가 모바일에서 접는다.** 반대로 하면 JS 실패 시 내용에 영영 못 간다
  (법안 상세 Zone 2 애니메이션과 같은 판단)
- 🔴 **첫 카드는 펼쳐 둔다** — 전부 접히면 "아무것도 없는 페이지" 로 보인다
- 🔴 **목차 칩이 접힌 카드를 먼저 펼친다.** 안 그러면 접힌 카드로 스크롤해 헤더만 보인다.
  목차 스크립트보다 **먼저 등록**해 캡처 단계에서 펼치고, 그다음 목차가 위치를 계산하게 한다
- ⚠️ `.card-body` 가 flex 라 `[hidden]` 만으로는 안 숨는다 → `display: none` 명시
  (`.ba-proposers-grid`·`.bf-rest`·`.sp-mts` 에서 반복해 겪은 함정)
- ⚠️ **`matchMedia` 의 `change` 만 믿지 말 것** — `resize` 도 같이 듣는다.
  실측: 브라우저 도구로 뷰포트를 바꾸면 **둘 다 발화하지 않는다**(실제 브라우저는 쏜다).
  `resize` 이벤트를 직접 쏘면 정상 동작 확인
- ⚠️ rAF 로 미루지 말 것 — 배경 탭에서 콜백이 안 돌아 초기 상태가 통째로 빠진다 (목차에서 이미 겪었다)
...
- 실측 (**2026-09-01** 소속 기반 재분류 후): 405쌍 / 코호트 158쌍(147명) · 평균 **51.8%** · 중앙값 53.6% · 평균 분모 48개.
  ⚠️ 구 값(2026-08-15: 388쌍 / 코호트 164쌍 · 평균 49.7% · 중앙값 50.7%)은 **정부 위원회 위원장의 답변이
  섞인 상태**였다 (위 `role_kind` 항목 참조). 평균이 2.1%p 오른 건 그 오염이 빠진 결과다.
  ...
  - 🔴 **닫힌 시트는 `visibility: hidden` 으로도 잠근다** (2026-08-16). `translateY(100%)` 만으로는 iOS Safari 에서
    ...
    - 같은 이유로 nav 모바일 패널 `.pb-mobile-panel`(main.css)도 닫힘 시 `visibility: hidden` + `height: 100dvh`.
      **화면 밖으로 밀어두는 fixed 요소는 전부 이 규칙** — 새로 만들 때 transform 만으로 숨기지 말 것.
      2026-08-16 375px 스윕 16페이지: 가로 오버플로 0, 닫힘 상태에서 보이는 fixed 없음
      (당시 예외였던 bill 상세 jumpbar 는 2026-08-24 에 제거됐다)
...
| `syncSpeeches.js` | 열린국회정보 (`npeslxqbanwkimebr`) | 발언영상 → `politician_speeches`. **연도 단위 UPSERT**(증분 인자가 없다) · 기본 올해분 · `--full` `--year N` `--dry-run`. 실측 올해분 6초 / 전건 63초. ⚠️ 로그의 **`[소속미상]`** 에 정당 이름이 뜨면 `PARTY_FALLBACK` 에 넣을 것 (기관명만 뜨는 게 정상) |
...
- 🔴 **본문 확보는 2단이다 — 크롤이 비면 `bills.summary` 로 폴백한다** (2026-08-23, `resolveContent`). pal 은 **입법예고 페이지**라 철회·폐기된 안의 본문이 빠져 있다. 실측: 박범계 법원조직법 개정안(철회)은 크롤 **0자**인데 `bills.summary` 에 752자가 있었고, 폴백 전까지 이런 법안은 `본문 없음 — 스킵` 으로 **영영 분석되지 않았다**
  - ⚠️ 두 소스는 **같은 문서라는 보장이 없다** (pal 입법예고 / 열린국회 `BPMBILLSUMMARY`). 그래서 화면에 `AI 가 읽은 원문` 같은 표현을 쓰면 안 된다 — 아래 「법안 원문 섹션」 주의와 같은 이유
  - ⚠️ 폴백 경로는 `stripSummaryHeading` 을 반드시 거친다. 안 거치면 `제안이유 및 주요내용` 머리말이 프롬프트에 섞인다
  - ⚠️ 어느 소스를 썼는지는 **로그로만** 남는다 (`본문 737자 (bills.summary 폴백)`). DB 컬럼을 두지 않았다 — 화면이 이미 '동일 문서 보장 없음' 을 전제하므로 구분이 표시를 바꾸지 않는다
...
### 진행 상태
- 2026-08-10 소유권 확인 ✅ · 검토 요청 ✅ · CMP 설정 ✅
- **2026-08-19 반려 — 「가치가 별로 없는 콘텐츠」.** 아래 대응 후 **2주 이상 지나** 재검토 요청할 것 (반복 반려 이력은 불리하다)
- 승인 후 액션은 위 "정치 중립성 리스크" 항목

### 🔴 AdSense 반려 원인과 대응 (2026-08-19) — 되돌리지 말 것
심사 봇 입장에서 이 사이트는 **"국회 원문을 복제한 얇은 페이지 18,000장"** 이었다:

| 봇이 본 것 | 근거 |
|---|---|
| 사이트맵 19,089 URL 중 **98%(18,741)가 법안 상세**, 그 본문은 `bills.summary` = assembly.go.kr 원문 그대로 | `utils/sitemap.js` 가 bills 전건을 실었다. 87%가 동명 법안 → 중복·복제·빈약 페이지 |
| 사이트 고유 텍스트가 있는 페이지는 극소수 | AI 분석 65~117건 · 브리핑 ~30장 · 커뮤니티 0건 · 정적 11 |
| **전 페이지 meta description 이 동일** | `layout.ejs` 하드코딩 → 템플릿 양산 사이트 |
| 브리핑 카드 전부가 canonical `/briefing` 을 선언 | `BriefingController` 가 상세에도 `currentUrl: '/briefing'` 을 넘겼다 → 유일한 고유 콘텐츠가 전부 "중복" 으로 접혔다 |
| 운영 9일 | 도메인 08-10 |

대응 (전부 코드):
1. **사이트맵 법안 = AI 분석 있는 것만** (`SOURCES.bills` 에 `JOIN bill_ai_analysis`)
2. **분석 없는 법안 상세는 `<meta name="robots" content="noindex, follow">`** — `BillController.getDetailPage` 가 `noindex: !hasAnalysis` 를 넘기고 layout 이 출력.
   `follow` 인 이유: 의원·브리핑으로 가는 내부 링크는 살아 있어야 한다. 🔴 **1과 2는 같은 조건이어야 한다** (사이트맵엔 있는데 noindex 면 모순 신호)
3. **페이지별 `pageDesc`** — layout 은 `pageDesc || ogDesc || 기본값`. 상세는 데이터로 조립(법안: AI 요약 or 발의자·날짜·위원회·결과 / 의원: 정당·지역구·선수·발의 건수·표결참여율), 목록·정적은 한 문장씩.
   ⚠️ 새 페이지를 만들면 `pageDesc` 를 넘길 것 — 안 넘기면 기본값으로 떨어져 다시 "같은 설명" 이 는다
...
"이 숫자를 어떻게 읽어야 하나" 를 글로 모았다. 1 반대표는 왜 1%도 안 될까 · 2 법안 하나가 국회를 통과하기까지 ·
3 대표발의와 공동발의, 건수가 말하지 않는 것 · 4 당을 보고 찍나 법안을 보고 찍나 · 5 의원 성향 좌표는 어떻게 만들었나.

```
utils/guideArticles.js            ← slug·title·sub·desc·minutes·related  (단일 소스)
views/guide/index.ejs             ← 목차 (번호 카드)
views/guide/article.ejs           ← 글 껍데기 (제목·본문 include·관련 화면·이전/다음)
views/guide/articles/<slug>.ejs   ← 본문 (facts 를 받는다)
public/styles/guide.css           ← .gd-*  (720px · 16px/1.85 · 세리프 제목 · 골드 단색)
```
- 🔴 **본문에 정확한 수치를 박지 말 것** — 배치가 매일 움직인다. 1편의 세 숫자는 컨트롤러가 `getHomeFacts`(10분 캐시)로 넘긴 `facts` 를 그리고,
  실패하면 `≈76` 처럼 "약" 으로 폴백한다 (글은 나간다). 나머지 글의 숫자는 전부 "안팎"·"가까이" 로 썼다 (소개 페이지 규칙)
- 🔴 **각 글 끝 「이 글이 말하지 않는 것」(`.gd-caveat`) 은 필수다.** 평가어·정당색 없음. "소신"·"거수기" 같은 말은 4편이 명시적으로 안 쓴다고 밝힌다
- 4편 구간표는 `GAP_BANDS`(app.locals) 에서 그린다 — 목록 필터·의원 상세와 같은 값
- 글 추가 = `GUIDE_ARTICLES` 에 한 항목 + `articles/<slug>.ejs`. 사이트맵(`STATIC_PATHS` 가 배열을 spread)·목차·이전/다음·description 은 자동
- 진입로: **nav `읽는 법`** (구 `용어` 자리 — 처음엔 안 넣기로 했다가 같은 날 "용어 메뉴 자리에 읽는 법, 그 안에 용어집" 으로 바꿈) ·
  홈 히어로 텍스트 링크 · 푸터(`읽는 법`·`용어`) · `/about` 「데이터 출처와 한계」 끝 문장
- 🔴 **용어 설명은 이 섹션의 두 번째 층이다** — `/guide/glossary` (구 `/glossary` 는 301). **상단 탭 `views/guide/_tabs.ejs`**
  (`[해설 N편 | 용어 설명]`, 링크 방식·JS 없음)를 `/guide` 와 `/guide/glossary` 가 같이 include 한다 — 사용자 지적: "내려보지 않아도 용어집이
  있는 걸 알아야". 두 페이지의 `.pb-page-title` 은 둘 다 `읽는 법`(탭이 어느 층인지 말한다). glossary 라우트는 `pageStyles: 'guide'` 로 탭 CSS 를 받는다.
...
- 🔴 **법안은 AI 분석이 있는 것만 싣는다** (2026-08-19, `JOIN bill_ai_analysis`). 그전엔 전건 18,741 이었고 그게 **AdSense 반려의 주원인**이었다 — 아래 「AdSense 반려」 참조.
  실측 **19,089 → 465 URL** (법안 117 · 의원 310 · 브리핑 30 · 정적 11)
- 🔴 **6시간 메모리 캐시 + inflight 공유는 필수다.** 크롤러가 반복 호출하면 **막으려던 그 부하가 여기서 다시 생긴다.** `XrayService` 섹션 캐시와 같은 수법 (전건 시절 18,000행 쿼리였다)
...
### 🔴 「말하지 않는 것」은 **공용 컴포넌트**다 — `.pb-caveat` · `.pb-note` (2026-08-24)
이 사이트의 차별점은 "데이터가 많다" 가 아니라 **"무엇을 모르는지 밝힌다"** 인데,
그 문구를 담는 클래스가 **여섯 갈래**로 흩어져 있었다 (`gd-caveat` 8곳 · `iss-caveat` ·
`kpi-caveat` · `pf-caveat` · `sp-caveat` · `pv-basis`). 같은 역할의 글이 자리마다 다르게 그려지면
**브랜드 장치가 아니라 그때그때의 잔소리로 읽힌다.** 정의는 `main.css` 맨 아래 한 곳.
...
| 형태 | 박스 (제목 필수) | 평문 |
| 자리 | 페이지·글 **끝** | 숫자 **바로 아래** |
| 역할 | "이 페이지가 말하지 않는 것" | 그 숫자의 **전제** |
| 쓰는 곳 | `/guide` 8편 · `/issue/:slug` | 의원 상세 4곳 · `_profile_vs` |

- 🔴 **인라인 각주를 박스로 승격하지 말 것** — 숫자에서 떨어지는 순간 전제가 아니게 된다.
  `.kpi-caveat` 는 KPI 행에 붙이려고 **음수 마진(-14px)** 까지 쓰고 있다
- 🔴 **쓰는 법은 기존 클래스를 지우지 말고 나란히 붙이는 것** (`class="pb-note kpi-caveat"`).
  공용이 **타이포만** 정하고 위치·여백은 자리의 지역 클래스가 그대로 갖는다
- 🔴 **공용 쪽에 복합 선택자(`ul.pb-note` 등)를 쓰지 말 것** — (0,1,1)이라 지역 클래스
  `.pf-caveat`(0,1,0)를 이겨 위 계약이 깨진다. 실제로 그렇게 썼다가 `margin-top: 14px` 이
  전부 0 이 됐다 (실측으로 잡음). 공용은 `.pb-note` 자신(0,1,0)에만 건다
- **통합하면서 드러난 것 세 가지** (전부 실측):
  | | |
  |---|---|
  | `/issue` 각주의 링크 2개가 **링크로 안 보였다** | 전역이 `a` 를 color:inherit·밑줄 없음으로 두는데 `.iss-caveat a` 규칙이 없었다 → 공용 `.pb-caveat a` 가 골드+밑줄 |
  | 같은 탭 안 두 각주의 **들여쓰기가 갈려 있었다** | `.pf-caveat` 14px vs `.sp-caveat` **54px** (후자가 ul 기본 padding 40px 을 안 껐다). "같은 모양이어야 한다" 는 주석만 있고 실제로는 달랐다 |
  | `.sp-caveat-tight` 가 **죽어 있었다** | 40줄 위에 있어 `.sp-caveat` 의 shorthand `margin` 에 덮였다 (의도 0/20 · 실제 14/0). 같은 특이도면 **파일 순서가 곧 우선순위** |
- ⚠️ `.pv-basis` 의 **11.5px 는 의도된 예외**다 (히어로 우측 패널이 320px 라 12px 로 올리면
  줄이 늘어 히어로 높이가 밀린다). 다른 각주를 11.5px 로 내리는 근거로 쓰지 말 것 —
  나머지는 **12px 하한**(문장형 설명 규칙)을 지킨다
- ⚠️ `/guide` 는 `.gd-caveat` 를 **지우지 않고 얇게** 남겼다 (`font-size` + 목록 3줄).
  guide.css 가 main.css 보다 뒤에 로드돼 같은 특이도면 `.gd-body ul` 이 공용을 이기기 때문 —
  그 세 줄을 지우면 각주 목록에 18px 바깥여백이 되살아난다. 실측 결과 **원본과 픽셀 동일**(높이 237·389)
- **쓰는 곳**: `/guide` 8편 · `/issue/:slug` · **`/bill/:id`**(2026-08-24 추가, 위 「이 페이지가 말하지 않는 것」 참조)
- ⚠️ **아직 각주 상자가 없는 주요 페이지**: `/xray` · `/briefing/:id` · `/politician/:id`(인라인 각주만).
  컴포넌트는 준비됐고 남은 건 **문구를 쓰는 일**이다 (기계적으로 붙일 게 아니라 페이지마다 내용이 다르다)
...
### 🔴 화면 문장에 em dash `—` 를 쓰지 않는다 (2026-08-19)
사용자 지적: "간간히 `—` 쓴 게 보이는데 너무 AI 틱하다". 화면에 나가는 문장·라벨·description·공유 텍스트 173곳을 전부 걷어냈다.
- **문장 중간 보충**은 문장을 끊거나(`. `) `,`/`:` 로 — 예: `값을 숨기지 않습니다 — 사실이라서` → `값을 숨기지 않습니다. 사실이니까요`
- **`제목 — 부제`·라벨 — 설명** 은 ` · ` (사이트 공통 구분자). 다만 그 줄에 `·` 가 이미 있으면 `:` 나 괄호
- **브랜드 타이틀**도 바꿨다: `당말사 — 당 말고 사람` → **`당말사 · 당 말고 사람`** (사용자 선택 A안 — 아래 타이틀 규약)
- ⚠️ **"값 없음" 표시의 `—`(`|| '—'`, `.em-dash`, 카운트 로딩 전 `<span>—</span>`)는 문장부호가 아니라 기호라 그대로 둔다.** 코드 주석·CSS 주석·EJS 주석은 대상이 아니다
- 점검 스크립트: 뷰·컨트롤러·서비스·utils·public/scripts 를 훑어 주석(`<%# %>`·`/* */`·`//`)을 걷어낸 뒤 `—` 가 남은 줄을 낸다.
  렌더 검증은 HTML 에서 `<style>`·`<script>`·`<!-- -->` 를 뺀 뒤 세는 것 (인라인 CSS 주석이 렌더에 그대로 실려 헷갈린다).
  새 문구를 쓸 때 `—` 가 손에 붙는다 — 이 목록이 다시 늘면 이 항목부터 볼 것

### 브라우저 타이틀 규약 (2026-08-10 · 2026-08-19 구분자 변경)
```
홈       →  당말사 · 당 말고 사람
그 외    →  {페이지 이름} · 당말사 · 당 말고 사람
```
- 구분자는 전부 ` · ` 다 (2026-08-19 까지는 브랜드↔태그라인 사이가 `—` 였다 — 위 「em dash」 항목). OG/Twitter title·로고 aria-label 도 같이 바뀌었다
...
- 길이 실측: 목록·랜딩 18~24자, **상세는 최장 74자** (아래에서 발의자·정당을 붙였다).
  넘치면 SERP 가 뒤를 자르는데 잘리는 건 브랜드 꼬리라 무해하다

#### 상세 페이지 제목은 **괄호로** 구분 요소를 붙인다 (2026-08-21)
`pageTitle` 에 이름만 넘기면 검색결과·탭·공유 미리보기에서 페이지가 구분되지 않는다.
`pageDesc` 만 갈라두는 것으로는 부족하다 — SERP 의 첫 줄은 제목이다.

| | 전 | 후 |
|---|---|---|
| 법안 상세 | `조세특례제한법 일부개정법률안` (동명 788건) | `조세특례제한법 일부개정법률안 (구자근 대표발의 · 2026.08.18)` |
| 의원 상세 | `강경숙` | `강경숙 의원 (조국혁신당 · 비례대표)` |

- 🔴 **괄호로 묶을 것.** 브랜드 꼬리(` · 당말사 · 당 말고 사람`)도 `·` 라서 그냥 이으면 점 사슬이 되어
  어디까지가 페이지 이름인지 안 보인다
- ⚠️ 법안의 `proposer_name` 은 의원(`mona_cd` 있음)일 수도 위원장·정부(없음)일 수도 있다 → `대표발의` / `제안` 으로 나눈다
- ⚠️ 의원 상세 309장은 **전부 색인 대상**이다 (분석 없는 법안 상세는 `noindex` — 위 AdSense 항목).
  그래서 제목 개선의 실익이 법안보다 크다

#### 🔴 목록 `?page=N` 은 크롤을 막아둔 상태다 — 열지 말 것 (2026-08-21 검토 후 보류)
`robots.txt` 의 `Disallow: /bill?` 가 필터 조합(크롤 트랩)과 함께 **페이지네이션도 막는다.**
그래서 크롤러가 목록 1페이지에서 더 못 가고, 링크로 닿는 법안은 600건 남짓이다.
`/bill/page/N` 같은 경로형으로 열면 18,000건 전부 도달하게 만들 수 있고 실제로 만들어 봤지만 **버렸다.**

- 🔴 2026-08-19 에 **분석 없는 법안 18,000장을 `noindex` + 사이트맵 제외**로 정리했다 (AdSense 반려 대응).
  거기에 크롤러를 보내면 **구글이 색인하지 않을 페이지를 크롤하는 데 예산을 쓴다** — 반려 사유를 다시 만드는 셈이다
- 색인 대상(분석 있는 법안 · 의원 309 · 브리핑)은 **사이트맵이 이미 발견을 담당**하므로 페이지네이션 개방이 필요 없다
- ⚠️ 두 정책은 세트다. 페이지네이션을 열려면 `noindex`·사이트맵 제외를 **같이** 되돌려야 하고,
  그건 AI 분석 커버리지가 지금의 **0.6%(116/18,741)** 에서 크게 올라 법안 본문이 사이트 고유 콘텐츠가 된 뒤의 이야기다
- ⚠️ 그때 열더라도 `robots.txt` 에 `Allow: /bill?page=` 를 넣는 방식은 쓰지 말 것 —
  `page` 가 쿼리 맨 뒤에 오는 것에 의존하는데 그건 `bill.ejs` `qsBuild` 의 키 순서일 뿐이라
  누가 재정렬하면 조용히 트랩이 열린다. 경로형(`/bill/page/N`)이라야 그 의존이 없다
