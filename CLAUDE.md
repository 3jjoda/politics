...
- 🔴 **상세에 앵커 내비 `.iss-toc` 가 있다** (2026-08-24). 섹션이 8개인데 훑을 수단이 없었다 —
  ...
  칩 문법·활성 추적 방식은 `.ov-toc` 와 같게 맞췄다. 고칠 일이 생기면 **두 곳을 같이 볼 것**
  ...
  - 🔴 **배경·보더는 바깥이 아니라 가운데 정렬된 `-inner` 에 준다.** 바깥에 주면
    **흰 띠만 화면 끝까지 뻗어** 본문과 따로 논다 (1920px 실측: 띠 1910 vs 본문 1000 —
    ...
    흰 배경·보더는 `-inner` 가 갖는다. 의원 상세 `.ov-toc` 도 마찬가지
  ...
  - 실측 2026-08-24: 칩 8개 690px(데스크톱 1000px 안에 한 줄 · 모바일 가로 스크롤) ·
    클릭 착지 후 섹션 상단이 고정 크롬 아래 12px ·
    **320·375·768·1024·1280·1440·1920px 전부** 제목=칩=본문 좌측 기준선 일치 · 문서 가로 오버플로 **0**
...
- ⚠️ **아직 각주 상자가 없는 주요 페이지**: `/bill/:id` · `/xray` · `/briefing/:id`.
