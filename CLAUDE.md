...
#### 🔴 생성 창 `--window` (기본 30일) — 과거로 무한히 파고들지 않게
`pickDays` 는 "카드 없는 **가장 최근** 발의일" 을 고른다. 창이 없으면 새 발의일이 없는 날마다
과거로 하나씩 내려간다 — 실측(2026-08-12) 발의일 537일 중 카드 없는 날이 **527일**(2024-05-30까지)이었다.
발의는 주 5일인데 크론은 매일 돌아 남는 실행이 전부 과거 백필에 쓰인다.
- 기본 `--window 30`. 창 안에 할 일이 없으면 `대상 날짜가 없습니다` 로 조용히 끝난다 (실패 아님)
- 새 발의일은 `ORDER BY propose_dt DESC` 라 **항상 우선** — 최신성은 창과 무관하게 유지된다
- 과거 백필은 의도적으로: `node batch/genBriefing.js --window 0 --limit 50`
- ⚠️ 창을 없앨 거면 **페이지네이션이 먼저다.** 피드는 20건씩 끊는데, 페이저가 없던 시절엔
