...
| `/admin/sns` | `admin/sns.ejs` | **관리자 — SNS 콘텐츠 허브** (2026-08-26, `requireAdmin`). 캐러셀 3종(`/promo/intro`·`numbers`·`balance`) + 쓰레드 문안 뱅크(`/promo/threads`) + 최근 브리핑 7건의 인스타 카드·쓰레드 체인 링크(폴백·활동없음은 `올리지 않는 카드` 표시) + 주간 루틴 요약. 🔴 도구 페이지 자체는 공개 라우트 — 여기는 찾아가는 메뉴지 권한 경계가 아니다. 전략 원문은 SNS.md. 진입: 계정 메뉴 `SNS 콘텐츠` |
