...
### 베타 오픈 전 필수
1. 여야 협력(bipartisan) 탭 실데이터 구현
2. 모바일 반응형 점검 (nav 로고는 완료, 나머지 페이지 필요)
3. footer 링크 연결 (`/privacy` 등 `#` 처리된 것들)
4. Google AdSense 신청
5. ~~OG 태그 적용 (og-image.png → layout.ejs)~~ ✅ 2026-04-23 완료
...
6. 국회 X레이 메뉴 (시각화 5종)
   ...
   - ④ 당론 이탈 의원 순위
   - ⑤ 법안 생존율 깔때기
7. AI 법안 분석 (가결 490건 먼저, 건당 $0.0004)
8. 정치 성향 밸런스 게임 + 밸런스게임 서비스 링크
9. 재화 시스템 (credits 컬럼 + credit_logs 테이블)
10. 의원 별점 → 재화 소비로 전환

### 베타 오픈 후 — 2순위
11. 악성 댓글 실시간 감지 + 삼진아웃
12. 의원 상세 개요 분석 탭 오픈
13. 커뮤니티 고도화 (신고·검색·정렬)
14. 실시간 알림
15. `bill_topic_cd` 컬럼 DB 드롭
