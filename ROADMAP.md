...
12. ~~**의원 활동 지표 확대 — 발언 데이터**~~ ✅ 2026-08-15 완료 (배치 복구 · `gov` 3분할 · 의원 상세 UI)
    - 유실됐던 코드를 전부 재작성했다. 상세는 [CHANGELOG.md](./CHANGELOG.md) 2026-08-15 (2), 운영 규칙은 [CLAUDE.md](./CLAUDE.md) `politician_speeches` 항목
    - 실측 **66,882행 / 의원 309명 / 2024-06-05~2026-08-14** · 파싱률 98.6% · `batch:daily` 편입 완료
    - 미해결이던 `gov` 오분류는 **귀속 문제**로 판명돼 3분할(`government`/`witness`/`other`)로 정리했다. 화면·집계는 `member`·`chair` 만 쓴다
    - 남은 것:
      - ✅ 2026-08-15 **소속기간 정규화 참여율** 추가 (`politician_committee_speech` MV). 의원 상세에 `소속 후 회의 100번 중 82번 — 82% (평균 50%)` 형태
      - ✅ 2026-08-15 **위원회 소속 이력** `politician_committee_history` 추가. `syncCommittees` 가 매일 스냅샷을 비교해 기록하고, MV 가 이력을 우선 쓰고 없으면 근사로 폴백한다(`start_exact`)
        - ⏳ **지금은 100% 근사다** (시드 477행). 원구성·사보임이 생길 때마다 정확해진다 — 진행률은 `refreshCommitteeSpeech` 로그의 `[시작일] 이력 기준 N/388`
        - ⚠️ 이력이 충분히 쌓이기 전에는 참여율이 **실제보다 후하다.** 화면이 그 사실을 문구로 밝히고 있고, 다 쌓이면 문구가 자동으로 사라진다
      - ⬜ **`/xray` 지표로 확장** — 참여율이 생겼으니 기술적으로는 가능해졌다. 다만 올리기 전에 확인할 것: ① 코호트가 164쌍뿐이다 (원구성 직후라 분모 미달이 55%) ② 국정감사·본회의가 분모에서 빠진다 ③ 순위표로 만드는 순간 위 "후한 값" 문제가 사람 사이 비교에 그대로 들어간다. **소속 이력이 쌓인 뒤가 안전하다**
      - ⬜ **`other`·`witness` 551행을 저장 자체에서 뺄지** — 지금은 저장하고 화면에서만 배제한다. "왜 못 쓰는 행인지" 를 남기는 값이라 뒀는데, 누군가 `role_kind` 를 안 보고 집계하면 그대로 사고가 된다
      - ⬜ **위원회 소속 이력이 없어 과거 귀속이 헐겁다** — `politician_committees` 는 현재 스냅샷이라 동명이인(박지원) 해소가 "지금 소속" 기준이다. 폐기한 126건의 일부는 이력이 있으면 살릴 수 있다
