...
12. ~~**의원 활동 지표 확대 — 발언 데이터**~~ ✅ 2026-08-15 완료 (배치 복구 · `gov` 3분할 · 의원 상세 UI)
    ...
    - 남은 것:
      ...
      - 🔴 **위원회 소속 이력 테이블이 다음 순서다.** 지금은 소속 시작을 "그 위원회 첫 발언일" 로 근사하는데, **첫 발언 전 침묵기가 분모에서 빠져 값이 실제보다 후하다.** `syncCommittees` 가 전체 교체 전에 스냅샷을 비교해 변경분만 기록하면 된다 (외부 호출 0회). 후반기 원구성 직후라 **지금 시작하면 이번 임기 내내 정확한 분모**를 갖는다
