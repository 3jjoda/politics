// C:\dev\politics\jobs\updateBillTopics.js (DB Topic 코드 동적 로드 반영 최종 버전)
...
const TOPIC_REVIEW_FILE = path.join(process.cwd(), 'data', 'new_topic_review.txt');

// --- [정의] Topic 키워드 (코드 내에 유지되며, ID는 DB에서 로드) ---
// 키워드의 Key는 DB code_name과 정확히 일치해야 합니다.
const TOPIC_KEYWORDS_RAW = {
    ...
    '안전/재난/소방': ['안전관리', '재난', '소방', '화재', '응급의료', '승강기안전', '방사능', '위험물', '경비업', '지진', '119', '사격장', '급경사지', '수상레저', '재해', '석면피해', '해사안전', '교통안전', '경범죄', '긴급구호', '다중이용업소', '화학물질', '전기통신금융사기', '가습기살균제', '소방장비', '경비업', '전기안전', '총포', '안전관리', '교제폭력범죄'],
    '통일/외교/남북': ['북한', '남북교류', '통일', '재외국민', '외교', '위안부', '강제동원', '북한이탈주민', '판문점', '개성공단', '한미동맹', '일본정부', '남북관계', '북핵', '미얀마', '우크라이나', '쿠르드', '해외이주', '국제연합', '대일항쟁기', '겨레말큰사전', '남북협력기금', '독립유공자', '6·25', '종전선언', '역사왜곡', '한·아랍재단', '북한주민 모자보건', '판문점선언'],
...
const SPECIAL_TOPIC_KEYWORDS_RAW = {
    '국회/행정절차': ['출석요구의건', '의사일정변경동의의건', '사임의건', '운영위원장', '법률제명약칭법안', '의안의비용추계', '국회운영', '출석요구', '의사일정', '정부위원', '국무총리'],
    '정치/외교 이벤트': ['탄핵소추안', '탄핵소추', '특별검사임명', '비상계엄', '사임촉구', '징계안', '결의안', '유네스코세계기록유산', '군함도', '납북', '워싱턴선언', '한미정상', '대통령배우자', '주가조작', '도이치모터스', '광화문한자현판', '유관순열사', '경술국치일', '거창사건'],
...
function classifyTopic(billName) {
    ...
    for (const topicName in TOPIC_ID_MAP) {
        ...
        // 정책 주제는 15개만 있으므로 99x 코드가 아닌 경우에만 분류 시도
        if (topic.id >= 1 && topic.id <= 15) {
            for (const keyword of topic.keywords) {
                if (processedName.includes(keyword)) {
                    return topic.id; // ✅ 정책 Topic ID 반환 (1~15)
                }
            }
        }
    }
    ...
    for (const topicName in TOPIC_ID_MAP) {
        ...
        if (topic.id >= 991 && topic.id <= 992) {
            for (const keyword of topic.keywords) {
                if (processedName.includes(keyword)) {
                    return topic.id; // ✅ 특수 Topic ID 반환 (991, 992)
                }
            }
        }
    }
...
async function runTopicUpdate() {
    ...
    try {
        ...
        TOPIC_ID_MAP = topicCodes.reduce((acc, code) => {
            ...
            if (keywordsRaw) {
                // 키워드 배열의 각 요소에 공백 제거 및 소문자 변환 적용
            ...
            // 특수 ID는 별도 저장 (classifyTopic 함수의 폴백(fallback)에 사용)
            if (code.code_id >= 991) {
        ...
        // 2. bills 테이블의 모든 bill_id와 bill_name을 가져옵니다.
        ...
        logger.warn(`- 신규 토픽 검토 필요 건수: ${newTopicCount}건 (코드 993: ${TOPIC_REVIEW_FILE} 파일 확인 요망)`);
