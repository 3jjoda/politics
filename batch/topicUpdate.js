// updateBillTopics.js (재분류 대기 코드 998 반영 최종 버전)
...
// --- [정의] Topic 키워드 (분류 로직을 위해 코드 내에 유지) ---
// Note: 키워드는 DB에서 로드된 ID와 동적으로 매핑됩니다.
const TOPIC_KEYWORDS_RAW = {
    '보건/복지/의료': ['국민건강보험', '의료', '치매', '환자', '간호', '장애인복지', '노인복지', '영유아보육', '국민연금', '기초연금', '정신건강', '응급의료', '후천성면역결핍', '희귀질환', '돌봄', '보건', '요양', '간호법', '소아청소년', '산모', '자살예방', '의료급여', '치매관리', '장기요양', '복지', '약사법', '건강', '구강보건', '노인복지법', '동물보호', '은둔형 외톨이', '장애인권리보장', '아토피질환', '장애인권리보장법', '장애인활동지원', '차별조항 정비', '위기임산부', '양육비'],
    ...
    '통일/외교/남북': ['북한', '남북교류', '통일', '재외국민', '외교', '위안부', '강제동원', '북한이탈주민', '판문점', '개성공단', '한미동맹', '일본정부', '남북관계', '북핵', '미얀마', '우크라이나', '쿠르드', '해외이주', '국제연합', '대일항쟁기', '겨레말큰사전', '남북협력기금', '독립유공자', '6·25', '종전선언', '역사왜곡', '한·아랍재단', '북한주민 모자보건', '판문점선언', '민주유공자', '미국산쇠고기'],
    ...
    '유통/소비자/공정': ['유통', '소비자', '공정거래', '하도급', '방문판매', '담배', '화장품', '대리점거래', '경품', '상품권', '약관', '전자상거래', '독점규제', '집단소송', '대규모유통업', '온라인플랫폼', '할부거래', '수입식품', '마약류', '전자상거래 등에서의 소비자보호', '소비자보호', '담배사업', '상표법', '지적재산', '전통시장', '상점가'],
};
...
let TOPIC_ID_MAP = {}; 
let SPECIAL_TOPIC_ID = {}; // 991, 992, 993, 998, 999 ID 저장용
...
function classifyTopic(billName) {
    ...
    return SPECIAL_TOPIC_ID['재분류 대기'] || 998; 
}
...
async function runTopicUpdate() {
    ...
    try {
        ...
        logger.info(`[File Init] 기존 검토 파일을 초기화했습니다.`);
        // ------------------------
        ...
        const RECLASSIFY_CODE = SPECIAL_TOPIC_ID['재분류 대기'] || 998;
        const [allBills] = await pool.execute(`
            SELECT bill_id, bill_name 
            FROM bills
            WHERE bill_topic_cd IS NULL OR bill_topic_cd = ?
        `, [RECLASSIFY_CODE]);
        ...
        for (const bill of allBills) {
            updatePromises.push(limit(async () => {
                ...
                // 5. 분류가 안 된 경우 (993) -> 재분류 대기 (998)로 변경
                if (newTopicCode === (SPECIAL_TOPIC_ID['일반법/기타'] || 993)) {
                    finalTopicCode = RECLASSIFY_CODE; // 993 대신 998 (재분류 대기)로 업데이트
                    await logNewTopicForReview(bill.bill_id, bill.bill_name);
                    newTopicCount++; // 993 -> 998로 전환되는 경우만 카운트 증가
                }

                // 6. 정책 주제(1~15)가 아니면 무조건 998로 전환
                if (newTopicCode > 15) { 
                    finalTopicCode = RECLASSIFY_CODE; // 998 (재분류 대기)로 업데이트
                    await logNewTopicForReview(bill.bill_id, bill.bill_name);
                    newTopicCount++; 
                }

                // 7. DB UPDATE 실행 (finalTopicCode 사용)
        ...
        logger.warn(`- 재분류 대기 (998) 건수: ${newTopicCount}건 ( ${TOPIC_REVIEW_FILE} 파일 확인 요망)`);
