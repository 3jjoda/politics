...
async function runMissingDetailSync() {
    ...
    try {
        ...
        const [missingBills] = await pool.execute(`
            SELECT bill_id, bill_no, age_cd
            FROM bills 
            WHERE bill_topic_cd = ?
            AND bill_no IS NOT NULL AND bill_name = '정보 수집 필요'
            ...
            updatePromises.push(limit(async () => {
                ...
                if (details && details.billName) {
                    ...
                    const [updateResult] = await pool.execute(`
                        UPDATE bills
                        SET bill_name = ?, bill_kind_cd = ?, age_cd = ?, age_name = ?, 
                            proposer_kind_cd = ?, proposer_name = ?, mona_cd = ?, 
                            co_proposer_count = ?, propose_dt = ?, committee = ?, 
                            committee_id = ?, proc_result_cd = ?, proc_result_name = ?, 
                            link_url = ?, bill_topic_cd = NULL  -- 보완 후 Topic 재분류를 위해 NULL로 초기화
                        WHERE bill_id = ?
                    `, [
