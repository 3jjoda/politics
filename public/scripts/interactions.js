...
(function () {
  ...
  /* ===================================================================
     ...
     브라우저가 **링크 드래그(주소를 끌어다 놓기)** 를 시작한다. 그러면 반투명 고스트가 붙고
     화면이 **멈춘 것처럼** 보이며 다음 클릭이 먹지 않는다 (2026-08-24 사용자 보고).
     스크롤하려고 살짝 끄는 동작에서도 나서 체감이 나쁘다.

     ⚠️ 인라인 텍스트 링크는 막지 않는다 — 문장 안의 링크를 드래그하는 건 보통 **텍스트 선택**이고,
        그것까지 막으면 본문을 복사할 수 없게 된다. `display` 가 inline 인 앵커는 통과시킨다.
     ⚠️ CSS `-webkit-user-drag: none` 은 파이어폭스가 지원하지 않아 여기서 이벤트로 막는다.
  =================================================================== */
  document.addEventListener('dragstart', (e) => {
    const a = e.target.closest?.('a[href]');
    if (!a) return;
    if (getComputedStyle(a).display === 'inline') return;   // 문장 속 링크 = 텍스트 선택
    e.preventDefault();
  });
})();
