...
질의석 건수 / 위원장석 건수 / 발언한 날 / 회의 종류 분포 / 최근 발언 영상 8건.
...
- 해석 주의 4줄을 숫자와 세트로 붙였다 (위원회별 회의 수 차이 · 위원장석은 사회 · 재생시간은 개인 발언시간 아님 · 정부측/외부인 제외)
- ⚠️ `.sp-kind-fill` 에 `min-width: 3px` — 1% 미만 구간(소위원회 4건 = 0.38%)이 폭 0px 으로 렌더돼 막대가 안 그려졌다

### 4. 크론 배선

`batch:daily` 에 `syncCommittees` **다음**으로 편입. 이 배치는 이름 매칭에 `politicians`,
동명이인 해소에 `politician_committees` 를 쓰므로 둘 다 최신이어야 귀속이 맞는다.
